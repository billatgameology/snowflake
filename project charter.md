...
How "quasi-static" is enforced (added v1.3, decision 0005). Quasi-static is an elliptic statement, not a timestep one, and the two attachment rules treat it differently by design. Under GGThreshold, diffusion is G–G's single masked-average pass per tick — that is their published dynamics, machinery fidelity, with no physical-time claim attached. Under LibbrechtKinetics, the field is relaxed between growth steps to a stated residual tolerance in a stated norm, with convergence tests; iteration counts scale like (L/Δx)² and large counts are expected, not evidence of a unit error. Physical time enters only through the interface update, bounded separately by the fill-CFL v_n·Δt/Δx.
...
Physical units: Δx in microns, Δt in seconds, D in m²/s. Physical time attaches to the interface update, bounded by the fill-CFL v_n·Δt/Δx; the vapor field itself is quasi-static — relaxed to a stated residual tolerance per growth step, with (L/Δx)²-scale iteration counts expected (§2.4). (v1.3, decision 0005: the v1.2 phrasing that derived a "principled number of diffusion iterations" from per-sweep physical time mixed transient and quasi-static formulations and is retracted.)
