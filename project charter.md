...
Project Document — v1.6, July 2026
...
How "quasi-static" is enforced (added v1.3, decision 0005; convergence criterion corrected v1.4, decision 0006). Quasi-static is an elliptic statement, not a timestep one, and the two attachment rules treat it differently by design. Under GGThreshold, diffusion is G–G's single masked-average pass per tick — that is their published dynamics, machinery fidelity, with no physical-time claim attached. Under LibbrechtKinetics with the fixed-σ Dirichlet condition used for physics runs, the field is relaxed between growth steps until BOTH a stated iterate-residual tolerance in a stated norm AND the divergence identity of the discrete solve (far-field injection equals actual discrete Robin absorption, to its own stated tolerance) are met — the identity is part of convergence itself, because residual-only convergence was measured passing fields whose imbalance grew with domain size (decision 0006). Reflecting LibbrechtKinetics mode is diagnostic-only: it has no far-field injection, makes no divergence-identity claim, and converges by residual alone. Iteration counts scale like (L/Δx)² and large counts are expected, not evidence of a unit error. Physical time enters only through the interface update, bounded separately by the fill-CFL on the per-cell kinetic fill increment (summed per attached face with the hexagonal-prism geometry factors — see Phase 2b; v1.4).
...
Phases 1 and 2 can run in parallel; everything else is sequential. (One recorded exception, 2026-07-15, decisions 0007 and 0008: Phase 3's development-visualization work overlapped the tail of Phase 2b's pre-registered evidence run and, by maker direction after that run exited with a recorded negative result, completes ahead of 2b's closure — Phase 3's gate rests only on Phase 2a machinery, and Phase 2b's gate still closes only on its own recorded result.) Each milestone has a done when.
...
Physical units: Δx in microns, Δt in seconds, D in m²/s. Physical time attaches to the interface update, bounded by the fill-CFL on the per-cell kinetic fill increment [(2/3)·n_T + n_Z]·alphaHK·v_kin·σ_face·Δt/Δx (per-face hexagonal-prism geometry — v1.4, decision 0006; the v1.3 scalar v_n·Δt/Δx predates the face factors); under fixed-σ Dirichlet, the vapor field itself is quasi-static — relaxed until both the stated residual tolerance and the divergence identity hold (v1.4, decision 0006), with (L/Δx)²-scale iteration counts expected (§2.4). Reflecting LibbrechtKinetics remains a residual-only diagnostic mode with no physical claim. (v1.3, decision 0005: the v1.2 phrasing that derived a "principled number of diffusion iterations" from per-sweep physical time mixed transient and quasi-static formulations and is retracted.)
...
The seam, which is the real work of this phase: G–G's attachment is a binary cell flip; Libbrecht's `v_n = alphaHK·v_kin·σ_surf` is a continuous velocity. Converting a surface velocity into lattice attachment events is the substance of LibbrechtKinetics, not an implementation detail of it. Reference implementation for the fill state (added v1.2; refined v1.3; face geometry v1.4, decision 0006): a deterministic fill-fraction accumulator on boundary cells — each step adds the per-face kinetic increment [(2/3)·n_T + n_Z]·alphaHK·v_kin·σ_face·Δt/Δx (a prism face fills a hexagonal-prism cell at 2/3 the basal rate; the uniform v_n·Δt/Δx of v1.2–v1.3 predates the face geometry and overdrove lateral growth 50%), and the cell freezes when fill ≥ 1. Where the fill lives and how it couples to the freezing/melting transfers is defined by the surface-operator spec (decision 0005 D2), whose exact claim is kinetic-demand bookkeeping — placed fill + recorded unapplied saturation excess = computed per-face Hertz–Knudsen kinetic demand, with shell-clamp totals as numerical diagnostics only (v1.4, decision 0006: audits found silent clipping and unphysical metered-source accounting hiding under v1.3's bare "vapor lost equals ice gained"); the clipping term is not deposited fill, ice, or physical uptake. v1.2's "reuses the boundary-mass machinery" was one candidate answer, not a decision. Accumulation is deterministic; stochasticity enters only through the single labeled noise dial — GGThreshold slows diffusion, LibbrechtKinetics slows `alphaHK` identically on sink and growth — never through stochastic rounding.
...
Conditions changing mid-growth: the timeline drives the real solver. Done when a plate→column history yields a capped column. Requires the timeline-semantics decision first (v1.3, decision 0005 D5): changing temperature changes c_sat(T), so the conserved field, what the timeline controls mean (absolute vapor density vs σ relative to ice), jump-vs-ramp behavior, and how the interior field and far-field reservoir transform at segment boundaries must be written down before this milestone's runs mean anything.
...
Protocol freeze (added v1.2 — pre-registration; expanded v1.3–v1.4, decisions 0005–0006). Before the first validation sweep runs, freeze docs/libbrecht-parameters.md and a written validation protocol: the T/σ grid; the far-field boundary condition (fixed-σ Dirichlet, per §2.4); the crystal size at which habit is measured — habit is size-dependent, so measuring at a stated maximum dimension is what keeps comparisons apples-to-apples; metric thresholds; domain budgets; and pressure, physical seed size, Δx, the fill-CFL bound, the diffusion residual tolerance and its norm, the divergence-identity tolerance, the relaxation-sweep cap, float precision, the parameter interpolation scheme, noise amplitude, seed-ensemble size, the model/code version (commit hash), and the uncertainty-reporting scheme. Any post-freeze edit to parameters or protocol requires a logged ADR and invalidates prior sweep results — the full sweep re-runs. This is what makes "a negative result is a result" survive contact with a disappointing plot.
