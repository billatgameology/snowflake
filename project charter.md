...
Project Document — v1.15, July 2026
...
Current revision. v1.15 (2026-07-23) — decision 0017: Phase 5's headless WebGPU evidence runtime is Playwright 1.61.1's lockfile-pinned Chromium revision 1228; the browser and headless paths use the same future solver package and WGSL, while the gate records the actual backend and complete runtime provenance on both hosts (§3.1). Decision 0016's RTX 3080 Windows / M4 Metal lanes, ≈8M-cell preview target, and bounded-dispatch requirement remain unchanged.
...
requires exact browser/backend/adapter provenance from both lanes.
...
Development hardware is split (ADRs 0002 and 0016): the primary host is a Windows desktop with an AMD Ryzen 7 5700G, 64 GB RAM, and an RTX 3080 with 10 GB dedicated VRAM; it carries Phase 5 development, the D3D12/Vulkan conformance lane, and the Phase 6 sweeps. The previously documented Apple M4 Mac is retained as the Metal conformance lane. Consequently the same frozen WGSL comparison protocol must pass on both Metal and the observed Windows D3D12/Vulkan backend before Phase 5's gate closes — a GPU solver validated on one backend is not validated. Every evidence bundle records the actual adapter, backend, operating system, driver where exposed, runtime, and commit. Bounded dispatches remain a portability requirement: macOS will not reproduce Windows' ≈2 s watchdog reset, so a dispatch that feels fine on Metal can still kill the tab on Windows.
...
Done when GPU and CPU runs agree within pre-registered tolerance on both backends (Metal and D3D12/Vulkan — ADRs 0002 and 0016) and the preview budget (≈8M cells) is interactively editable. (Amended v1.2: cell-budget phrasing per ADR 0001; hardware lanes and tolerance pre-registration clarified v1.14 by ADR 0016.)
