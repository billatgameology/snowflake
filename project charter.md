...
How "quasi-static" is enforced (added v1.3, decision 0005; convergence criterion corrected v1.4, decision 0006). Quasi-static is an elliptic statement, not a timestep one, and the two attachment rules treat it differently by design. Under GGThreshold, diffusion is G–G's single masked-average pass per tick — that is their published dynamics, machinery fidelity, with no physical-time claim attached. Under LibbrechtKinetics, the field is relaxed between growth steps until BOTH a stated iterate-residual tolerance in a stated norm AND the divergence identity of the discrete solve (far-field injection equals surface absorption, to its own stated tolerance) are met — the identity is part of convergence itself, because residual-only convergence was measured passing fields whose imbalance grew with domain size (decision 0006). Iteration counts scale like (L/Δx)² and large counts are expected, not evidence of a unit error. Physical time enters only through the interface update, bounded separately by the fill-CFL on the per-cell kinetic fill increment (summed per attached face with the hexagonal-prism geometry factors — see Phase 2b; v1.4).
...
Phase 2b — attachment becomes physics. Paused as of v1.3 (decision 0005) until its two opening deliverables exist: the surface-operator specification (D2 in the ADR — normalization of d→σ, surface sampling, facet classification, vapor flux and ice gain coupled as one operator, the fill state and its storage, and the explicit kept/replaced/disabled disposition of κ, μ, melting, and hole-filling) and the parameter table below. No LibbrechtKinetics code before both.


Put attachment behind an AttachmentRule interface with two implementations, GGThreshold (2a's, unchanged) and LibbrechtKinetics. Both are kept permanently, exactly as the CPU oracle is kept permanently: the threshold rule is the working floor and the differential diagnosis when the physics misbehaves. The interface is a surface-operator interface — wider than a per-cell shouldAttach: it owns per-cell state and mediates the vapor↔ice mass exchange (v1.3).
Physical units: Δx in microns, Δt in seconds, D in m²/s. Physical time attaches to the interface update, bounded by the fill-CFL on the per-cell kinetic fill increment [(2/3)·n_T + n_Z]·alphaHK·v_kin·σ_face·Δt/Δx (per-face hexagonal-prism geometry — v1.4, decision 0006; the v1.3 scalar v_n·Δt/Δx predates the face factors); the vapor field itself is quasi-static — relaxed until both the stated residual tolerance and the divergence identity hold (v1.4, decision 0006), with (L/Δx)²-scale iteration counts expected (§2.4). (v1.3, decision 0005: the v1.2 phrasing that derived a "principled number of diffusion iterations" from per-sweep physical time mixed transient and quasi-static formulations and is retracted.)
...
Protocol freeze (added v1.2 — pre-registration; expanded v1.3, decision 0005). Before the first validation sweep runs, freeze docs/libbrecht-parameters.md and a written validation protocol: the T/σ grid; the far-field boundary condition (fixed-σ Dirichlet, per §2.4); the crystal size at which habit is measured — habit is size-dependent, so measuring at a stated maximum dimension is what keeps comparisons apples-to-apples; metric thresholds; domain budgets; and (v1.3) pressure, physical seed size, Δx, the fill-CFL bound, the diffusion residual tolerance and its norm, float precision, the parameter interpolation scheme, noise amplitude, seed-ensemble size, the model/code version (commit hash), and the uncertainty-reporting scheme. Any post-freeze edit to parameters or protocol requires a logged ADR and invalidates prior sweep results — the full sweep re-runs. This is what makes "a negative result is a result" survive contact with a disappointing plot.
