...
describe("named crystal direct-production review", () => {
  ...
  it("fills exactly the reviewed 24 catalog slots with matching entry identities and byte counts", () => {
    const review = JSON.parse(readFileSync(REVIEW, "utf8")) as ReviewWire;
    const catalog = parseNamedCrystalCatalog(JSON.parse(readFileSync(CATALOG, "utf8")) as unknown);
    const reviewed = new Map(
      review.families.flatMap((family) =>
        family.variants.map((variant) => [variant.jobId, variant] as const),
      ),
    );
    ...
    for (const variant of accepted) {
      const bound = reviewed.get(variant.entryId);
      expect(bound).toBeDefined();
      expect(variant.webPayloadBytes).toBe(bound!.webByteLength);
      expect(variant.links.webAsset).toContain(`/direct-production-v1/${variant.entryId}/growth-v1.bin`);
      expect(variant.links.scientificBundle).toContain(`/direct-production-v1/${variant.entryId}/`);
    }
  });
});
