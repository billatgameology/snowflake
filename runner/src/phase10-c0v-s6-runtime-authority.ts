...
export function derivePhase10C0VS6RetainedRuntimeAuthority(
  ...
  const authority = parsePhase10C0VS6RecoveryV7Authority(
    parsePhase10C0VS6PrettyJsonBytes(authorityBytes, "current recovery-v7 authority"),
  );
  const catalogue = parsePhase10C0VS6PacketCatalogue(parsePhase10C0VS6PrettyJsonBytes(
    ...
    "current recovery-v7 packet catalogue",
  ));
  if (authority.successor.packetCatalogueId !== catalogue.catalogueId ||
    ...
    fail("current recovery-v7 authority successor differs from the live packet catalogue");
  }
  return retained;
}
 ...
 * through the live recovery-v7 predecessor chain; current recovery-v7 packets retain the
 * ordinary live proof.
 */
export function derivePhase10C0VS6HistoricalRetainedRuntimeAuthority(
  input: Phase10C0VS6RawRuntimeAuthorityInput,
): Phase10C0VS6RetainedRuntimeAuthority {
  if (input.packetProtocolIdentity.path.startsWith(`${PHASE10_C0V_S6_RECOVERY_V7_AUTHORITY_ROOT}/`)) {
