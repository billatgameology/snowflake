...
const RECOVERY_AUTHORITY_PATH = PHASE10_C0V_S6_RECOVERY_V9_AUTHORITY_PATH;
...
function sourceIds(root: string): readonly IndexSource[] {
  ...
  register("authority-execution-v2-recovery-v9", RECOVERY_AUTHORITY_PATH);
  ...
  for (const packetId of [
    ...
    register(`authority-${packetId}-protocol`, `${PHASE10_C0V_S6_RECOVERY_V9_AUTHORITY_ROOT}/packets/${packetId}/protocol.json`);
    register(`authority-${packetId}-callable-registry`, `${PHASE10_C0V_S6_RECOVERY_V9_AUTHORITY_ROOT}/packets/${packetId}/callable-registry.json`);
  }
