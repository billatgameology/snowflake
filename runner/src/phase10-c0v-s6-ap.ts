...
export interface Phase10C0VS6ApArtifactIndex {
  ...
  readonly bundleId: "phase10-obligation-preflight-v6";
...
const RECOVERY_AUTHORITY_PATH = PHASE10_C0V_S6_RECOVERY_V8_AUTHORITY_PATH;
...
function sourceIds(root: string): readonly IndexSource[] {
  ...
  register("authority-execution-v2-recovery-v8", RECOVERY_AUTHORITY_PATH);
  ...
  for (const packetId of [
    ...
    register(`authority-${packetId}-protocol`, `${PHASE10_C0V_S6_RECOVERY_V8_AUTHORITY_ROOT}/packets/${packetId}/protocol.json`);
    register(`authority-${packetId}-callable-registry`, `${PHASE10_C0V_S6_RECOVERY_V8_AUTHORITY_ROOT}/packets/${packetId}/callable-registry.json`);
  }
