...
function assertSemanticCandidatePublicationJoin(
  ...
  lifecycle: Phase10C0VS6RawTerminalCandidateProjection,
  publicationArtifacts: readonly Phase10C0VS6ReopenedPublishedArtifact[],
  reproof: Phase10C0VS6HistoricalPacketSemanticReproof,
): void {
...
function constructMovingAttempt(
  root: Phase10C0VS6PhysicalRoot,
  candidate: Phase10C0VS6RawTerminalCandidateProjection,
  scanned: readonly Phase10C0VS6ReopenedPublishedArtifact[],
): Phase10C0VS6AttemptRowV2 {
  const packet = candidate.lifecycle.packet;
  if (packet.packetId !== "c0v-moving-produce") fail("moving attempt constructor received another packet");
  ...
  if (packet.bindings.scienceProtocol === null || packet.bindings.referenceOrRefusal === null) {
    fail("moving packet lacks its exact science/reference bindings");
  }
  ...
  if (tuples.length !== 1) fail("moving lifecycle route lacks one exact execution tuple");
  ...
  exactRoster(scanned.map((entry) => entry.identity.path), expectedAttemptPaths,
    "moving pre-ledger attempt-root census");
  ...
  if (resourceRosters.length !== 1 || resourceRosters[0]!.observationPointIds.length !== 1) {
    fail("moving tuple lacks the sole terminal-retention observation point");
  }
  ...
  phase10C0VS6SameJson(
    ...
    "moving raw worker timing versus lifecycle invocation records",
  );
  ...
  const concurrentBytes = safeIntegerSum(artifacts.map((entry) => entry.byteLength),
    "moving terminal retained bytes");
  ...
  const elapsed = safeIntegerSum(candidate.lifecycle.produceInvocationRecords.map((entry) =>
    entry.elapsedNanoseconds), "moving governed invocation elapsed nanoseconds");
  const classificationValidation = produceClassificationValidation(candidate, tuple, partialExecution);
  const run = packet.commandTemplates.filter((entry) => entry.commandId === "run");
  if (run.length !== 1) fail("moving packet lacks one exact run command");
  const attempt = parsePhase10C0VS6AttemptRowV2(Object.freeze({
    ...
    preflight: candidate.lifecycle.preflightIdentity,
    stdout: artifactFromAttemptScan(scanned, preflight.observed.stdoutPath, "moving stdout"),
    stderr: artifactFromAttemptScan(scanned, preflight.observed.stderrPath, "moving stderr"),
    terminalCandidate: candidate.candidateIdentity,
...
/**
 * Raw one-way moving-produce finalizer. The caller supplies only active lock/watchdog authority and
 * the exact JSON-only result captured from the governed moving check-caller leaf. Route, attempt,
...
export function independentlyFinalizePhase10C0VS6MovingProducePacket(
  ...
  phase10C0VS6SameIdentity(
    input.lockedAuthority.packetProtocolIdentity,
    input.packetProtocolIdentity,
    "moving finalizer locked packet protocol",
  );
  if (!sameBytes(input.lockedAuthority.packetProtocolBytes, input.packetProtocolBytes)) {
    fail("moving finalizer packet bytes differ from active locked authority");
  }
  const retained = derivePhase10C0VS6RetainedRuntimeAuthority(input);
  const packet = retained.packet;
  if (packet.packetId !== "c0v-moving-produce" || input.lockedAuthority.packet.packetId !== packet.packetId ||
    input.lockedAuthority.packet.protocolId !== packet.protocolId ||
    input.lockedAuthority.packet.registeredAttemptId !== packet.registeredAttemptId) {
    fail("moving finalizer received a different packet/locked authority");
  }
  const priorPrefix = independentlyReopenPhase10C0VS6VerifiedProduceDependencies(input);
  exactRoster(priorPrefix.selectedPackets.map((entry) => entry.packet.packetId), packet.boundDependencyPacketIds,
    "moving finalizer deeply verified dependency prefix");
  const authority = reopenCurrentPacketAuthority(input, packet, retained.preflight);
  const materialized = independentlyMaterializePhase10C0VS6TerminalCandidate(
    input,
    input.capturedGovernedCallerResult,
  ...
  if (existsSync(resolve(root.path, materialized.candidatePath))) {
    fail("moving terminal candidate existed before current raw finalization");
  }
  const candidate = materialized;
  const scanned = projectTerminalCandidateIntoAttemptScan(
  ...
  assertAttemptRootByteLimits(authority.catalogue, candidate, scanned);
  const attempt = constructMovingAttempt(root, candidate, scanned);
  const attemptReceipt = writePhase10C0VMovingAttemptReceipt({
  ...
  const callerRosters = packet.terminalReceiptContract.callerInvocationResultRosters.filter((entry) =>
    entry.subrouteId === candidate.lifecycle.selectedSubrouteId);
  if (callerRosters.length !== 1) fail("moving selected route lacks one full caller roster");
  exactRoster(callerRows.map((entry) => entry.callerInvocationId),
    ...
    "moving full caller invocation roster");
  ...
  phase10C0VS6SameIdentity(ledgerIdentity, attemptReceipt.identity, "moving projected attempt ledger");
  const inMemoryRows = parsePhase10C0VS6AttemptLedgerV2(attemptReceipt.bytes, "moving in-memory attempt ledger");
  if (inMemoryRows.length !== 1) fail("moving in-memory ledger does not contain its sole attempt");
  phase10C0VS6SameJson(inMemoryRows[0]!, attempt, "moving in-memory ledger attempt row");
  ...
  const subroutes = packet.terminalSubroutes.filter((entry) =>
    entry.subrouteId === candidate.lifecycle.selectedSubrouteId);
  if (subroutes.length !== 1) fail("moving selected lifecycle route disappeared from packet authority");
  const subroute = subroutes[0]!;
  ...
  const terminalProjection = finalizationProjection(packet, "terminal-receipt");
  if (terminalProjection.path !== packet.paths.terminalReceiptPath) {
    fail("moving terminal finalization path differs from packet terminal path");
  }
  const terminalIdentity = assertFreshPublicationBytes(
    root,
    packet,
  ...
  phase10C0VS6SameIdentity(publishedLedgerIdentity, ledgerIdentity, "moving published attempt ledger");
  const reopenedLedgerBytes = phase10C0VS6ReadUniquePhysicalFile(root, ledgerIdentity.path);
  const reopenedRows = parsePhase10C0VS6AttemptLedgerV2(reopenedLedgerBytes, "moving published attempt ledger");
  if (reopenedRows.length !== 1) fail("moving published ledger does not contain its sole attempt");
  phase10C0VS6SameJson(reopenedRows[0]!, attempt, "moving published ledger attempt row");
  ...
  if (verificationIdentity !== null && verificationBytes !== null && verificationAuthority !== null) {
    ...
    phase10C0VS6SameIdentity(
      ...
      verificationIdentity,
      "moving published packet verification",
    );
    verification = parsePhase10C0VS6PacketVerificationV2Bytes(
    ...
    fail("moving verification projection is only partially materialized");
  }
  ...
  phase10C0VS6SameIdentity(publishedTerminalIdentity, terminalIdentity, "moving published terminal receipt");
...
/**
 * Raw one-way moving-publication finalizer. The structural producer may only leave its result and
 * artifact-index bytes in the registered candidate directory. This function independently
 * reopens the complete A-P/moving-produce prefix, rederives the publication semantics, binds the
...
export function independentlyFinalizePhase10C0VS6MovingPublishPacket(
  ...
  phase10C0VS6SameIdentity(
    input.lockedAuthority.packetProtocolIdentity,
    input.packetProtocolIdentity,
    "moving-publish finalizer locked packet protocol",
  );
  if (!sameBytes(input.lockedAuthority.packetProtocolBytes, input.packetProtocolBytes)) {
    fail("moving-publish finalizer packet bytes differ from active locked authority");
  }
  const retained = derivePhase10C0VS6RetainedRuntimeAuthority(input);
  const packet = retained.packet;
  if (packet.packetId !== "c0v-moving-publish" || input.lockedAuthority.packet.packetId !== packet.packetId ||
    input.lockedAuthority.packet.protocolId !== packet.protocolId ||
    input.lockedAuthority.packet.registeredAttemptId !== packet.registeredAttemptId) {
    fail("moving-publish finalizer received a different packet/locked authority");
  }
  const priorPrefix = independentlyReopenPhase10C0VS6VerifiedProduceDependencies(input);
  exactRoster(
    priorPrefix.selectedPackets.map((entry) => entry.packet.packetId),
    packet.boundDependencyPacketIds,
    "moving-publish deeply verified dependency prefix",
  );
  const verifiedProduce = priorPrefix.byPacketId.get("c0v-moving-produce");
  if (verifiedProduce === undefined || verifiedProduce.selectedAttempt === null ||
    ...
    fail("moving-publish dependency prefix lacks its verified moving attempt/ledger");
  }
  const authority = reopenCurrentPacketAuthority(input, packet, retained.preflight);
  const materialized = independentlyMaterializePhase10C0VS6TerminalCandidate(
    input,
  ...
  if (existsSync(resolve(root.path, materialized.candidatePath))) {
    fail("moving-publish terminal candidate existed before current raw finalization");
  }
  const candidate = materialized;
  const scanned = projectTerminalCandidateIntoAttemptScan(
    scanAttemptRoot(root, retained.preflight.observed.attemptDirectory),
  ...
  exactRoster(
    ...
    "moving-publish finalized attempt-root census",
  );
  const subroutes = packet.terminalSubroutes.filter((entry) =>
    entry.subrouteId === candidate.lifecycle.selectedSubrouteId);
  if (subroutes.length !== 1) fail("moving-publish lifecycle route disappeared from packet authority");
  const subroute = subroutes[0]!;
  const acceptedPacketCredit = subroute.dispositionCode === null &&
  ...
  const callerRosters = packet.terminalReceiptContract.callerInvocationResultRosters.filter((entry) =>
    entry.subrouteId === candidate.lifecycle.selectedSubrouteId);
  if (callerRosters.length !== 1) fail("moving-publish route lacks one caller-result roster");
  exactRoster(
    callerRows.map((entry) => entry.callerInvocationId),
    callerRosters[0]!.callerInvocationResults.map((entry) => entry.callerInvocationId),
    "moving-publish full caller invocation roster",
  );
  ...
  let verificationAuthority: Phase10C0VS6PacketVerificationV2Authority | null = null;
  if (acceptedPacketCredit) {
    const resultRows = publicationArtifacts.filter((entry) => entry.outputId === "out-c0v-moving-result");
    const indexRows = publicationArtifacts.filter((entry) =>
      entry.outputId === "out-c0v-moving-artifact-index");
    if (resultRows.length !== 1 || indexRows.length !== 1) {
      fail("moving-publish completion lacks one in-memory result and artifact index");
    }
    const evaluation = independentlyEvaluatePhase10C0VMovingPublicationSemantic(Object.freeze({
    ...
    if (evaluation.aggregateVerdict !== "pass" || evaluation.packetId !== packet.packetId ||
      ...
      fail("moving-publish independent semantic reproof did not pass cleanly");
    }
    phase10C0VS6SameIdentity(
      evaluation.resultIdentity,
      resultRows[0]!.identity,
      "moving-publish semantic result identity",
    );
    phase10C0VS6SameIdentity(
      evaluation.artifactIndexIdentity,
      indexRows[0]!.identity,
      "moving-publish semantic artifact-index identity",
    );
    const checkIds = Object.freeze(evaluation.checkResults.map((entry) => entry.checkId));
    const semanticReproof: Phase10C0VS6HistoricalPacketSemanticReproof = Object.freeze({
      ...
      candidatePublishedOutputs: Object.freeze([
        Object.freeze({ outputId: "out-c0v-moving-artifact-index", identity: evaluation.artifactIndexIdentity }),
        Object.freeze({ outputId: "out-c0v-moving-result", identity: evaluation.resultIdentity }),
      ]),
    });
    ...
    phase10C0VS6SameJson(checkResults, evaluation.checkResults,
      "moving-publish verification checks versus semantic reproof");
    const execution = independentlyConstructProduceExecution(
      packet,
      candidate.lifecycle.selectedSubrouteId,
      retained.preflight,
    ...
    verificationBytes = writePhase10C0VMovingPublishVerificationReceipt(
      value,
      packet,
      verificationAuthority,
    );
    ...
    fail("moving-publish maker-return route retained a forbidden result/index candidate");
  }
  ...
  const terminalProjection = finalizationProjection(packet, "terminal-receipt");
  if (terminalProjection.path !== packet.paths.terminalReceiptPath) {
    fail("moving-publish terminal finalization path differs from packet terminal path");
  }
  const terminalIdentity = assertFreshPublicationBytes(
    root,
    packet,
  ...
  if (verificationIdentity !== null && verificationBytes !== null && verificationAuthority !== null) {
    ...
    phase10C0VS6SameIdentity(
      publishedVerification,
      verificationIdentity,
      "moving-publish published packet verification",
    );
    verification = parsePhase10C0VS6PacketVerificationV2Bytes(
      phase10C0VS6ReadUniquePhysicalFile(root, verificationIdentity.path),
      packet,
    ...
    fail("moving-publish verification projection is only partially materialized");
  }
  const publishedTerminal = publishFreshBytes(
    input,
    root,
  ...
  phase10C0VS6SameIdentity(publishedTerminal, terminalIdentity, "moving-publish published terminal receipt");
  terminal = parsePhase10C0VS6TerminalReceiptV2Bytes(
    phase10C0VS6ReadUniquePhysicalFile(root, terminalIdentity.path),
    packet,
    terminalAuthority,
...
function verifyCorePacket(
  ...
  } else if (packetId === "c0v-moving-publish") {
    if (semanticReproof === null) fail("moving publication lacks its independent semantic reproof");
    assertSemanticCandidatePublicationJoin(authority, candidate, publicationArtifacts, semanticReproof);
...
/**
 * Pure raw moving-publication reproof used by both observed and live prefix verification.  It
 * reopens the historical candidate and its own deeply verified A-P/moving-produce prefix before
 * invoking the cycle-free semantic leaf; a caller cannot supply either a semantic token or a
 * passing roster.
 */
function independentlyReproveMovingPublicationPrefix(
  input: Phase10C0VS6RawRuntimeAuthorityInput,
  current: Readonly<{
    readonly packet: Phase10C0VS6PacketProtocol;
    readonly preflight: Phase10C0VS6RetainedPreflight;
  ...
  const catalogueBytes = phase10C0VS6ReadUniquePhysicalFile(
    root,
    current.packet.bindings.packetCatalogue.path,
  );
  phase10C0VS6SameIdentity(
    ...
    "published-prefix packet catalogue",
  );
  const catalogue = parsePhase10C0VS6PacketCatalogue(
    parsePhase10C0VS6PrettyJsonBytes(catalogueBytes, "published-prefix packet catalogue"),
  );
  const movingRows = catalogue.packets.filter((entry) => entry.packetId === "c0v-moving-publish");
  if (movingRows.length !== 1) fail("catalogue does not resolve one moving publication packet");
  const movingProtocolBytes = phase10C0VS6ReadUniquePhysicalFile(root, movingRows[0]!.protocolPath);
  const movingProtocolIdentity = phase10C0VS6ArtifactIdentity(
    movingRows[0]!.protocolPath,
    movingProtocolBytes,
  );
  const movingPacket = parsePhase10C0VS6PacketProtocol(
    parsePhase10C0VS6PrettyJsonBytes(movingProtocolBytes, "historical moving publication protocol"),
  );
  if (movingPacket.packetId !== "c0v-moving-publish" ||
    movingPacket.paths.preflightReceiptPath !== movingRows[0]!.preflightReceiptPath ||
    movingPacket.paths.terminalReceiptPath !== movingRows[0]!.terminalReceiptPath) {
    fail("moving publication protocol differs from its exact catalogue row");
  }
  const movingPreflight = selectedPrefixDependencyArtifact(
    shallow.artifacts,
    "c0v-moving-publish",
    "phase10-c0v-s6-preflight-receipt-v2",
    "retained preflight",
  );
  const movingPreflightBytes = phase10C0VS6ReadUniquePhysicalFile(root, movingPreflight.identity.path);
  phase10C0VS6SameIdentity(
    phase10C0VS6ArtifactIdentity(movingPreflight.identity.path, movingPreflightBytes),
    movingPreflight.identity,
    "historical moving publication preflight",
  );
  const movingInput: Phase10C0VS6RawRuntimeAuthorityInput = Object.freeze({
    repositoryRoot: root.path,
    packetProtocolIdentity: movingProtocolIdentity,
    packetProtocolBytes: movingProtocolBytes,
    preflightBytes: movingPreflightBytes,
  });
  const candidate = independentlyReopenPhase10C0VS6HistoricalTerminalCandidate(movingInput);
  if (candidate.lifecycle.selectedSubrouteId !== "c0v-moving-publish-structural-complete" ||
    candidate.lifecycle.dispositionCode !== null || candidate.lifecycle.terminalState !== "complete") {
    fail("moving publication dependency did not select its exact structural-complete route");
  }
  const filenames = movingPacket.candidateFilenameRosters[candidate.lifecycle.selectedSubrouteId];
  if (filenames === undefined) fail("moving publication completion lacks a candidate filename roster");
  const resultBytes = phase10C0VS6ReadUniquePhysicalFile(
    root,
    `${candidate.lifecycle.preflight.observed.candidateDirectory}/${
      exactCandidateFilename(filenames, "c0v-moving-result.json")}`,
  );
  const artifactIndexBytes = phase10C0VS6ReadUniquePhysicalFile(
    root,
    `${candidate.lifecycle.preflight.observed.candidateDirectory}/${
      exactCandidateFilename(filenames, "c0v-moving-artifact-index.json")}`,
  );
  const movingDependencies = independentlyReopenPhase10C0VS6HistoricalVerifiedProduceDependencies(
    movingInput,
  );
  const produce = movingDependencies.byPacketId.get("c0v-moving-produce");
  if (produce === undefined || movingDependencies.selectedPackets.length !== 2 ||
    movingDependencies.selectedPackets[0]?.packet.packetId !== "a-p-c0v-s6" ||
    movingDependencies.selectedPackets[1]?.packet.packetId !== "c0v-moving-produce") {
    fail("moving publication semantic reproof lacks its exact deeply verified produce prefix");
  }
  const evaluation = independentlyEvaluatePhase10C0VMovingPublicationSemantic(Object.freeze({
    publicationPacket: movingPacket,
    verifiedProduce: produce,
    candidate: Object.freeze({ resultBytes, artifactIndexBytes }),
  }));
  if (evaluation.packetId !== "c0v-moving-publish" || evaluation.aggregateVerdict !== "pass") {
    fail("moving publication independent semantic reproof did not return its exact passing evaluation");
  }
  const checkIds = Object.freeze(evaluation.checkResults.map((entry) => entry.checkId));
  if (evaluation.checkResults.some((entry) => entry.verdict !== "pass" || entry.reasons.length !== 0) ||
    ...
    fail("moving publication independent semantic check roster differs");
  }
  return Object.freeze({
    packetId: "c0v-moving-publish",
    callerCallableId: "phase10-c0v-moving-publish-check-caller",
    ...
    candidatePublishedOutputs: Object.freeze([
      Object.freeze({
        outputId: "out-c0v-moving-artifact-index",
        identity: evaluation.artifactIndexIdentity,
      }),
      Object.freeze({
        outputId: "out-c0v-moving-result",
        identity: evaluation.resultIdentity,
      }),
    ]),
  });
}
...
function verifyProduceDependencyPrefix(
  ...
  const requiredCoreIds = current.packet.boundDependencyPacketIds.filter(
    (packetId): packetId is Phase10C0VS6DeeplyVerifiedPacketId =>
      packetId === "a-p-c0v-s6" || packetId === "c0v-moving-produce" ||
      packetId === "c0v-radial-produce" ||
      (packetId === "c0v-moving-publish" && reproofPacketIds.includes(packetId)),
  );
  if (requiredCoreIds.length !== current.packet.boundDependencyPacketIds.length ||
    requiredCoreIds.length === 0 || requiredCoreIds[0] !== "a-p-c0v-s6" ||
    (requiredCoreIds.includes("c0v-moving-produce") &&
      requiredCoreIds.indexOf("c0v-moving-produce") !== 1) ||
    (requiredCoreIds.includes("c0v-moving-publish") &&
      requiredCoreIds.indexOf("c0v-moving-publish") !== 2) ||
    (requiredCoreIds.includes("c0v-radial-produce") &&
      requiredCoreIds.indexOf("c0v-radial-produce") !== 3) ||
    semanticReproofs.some((entry) => !requiredCoreIds.includes(entry.packetId))) {
    fail(`${current.packet.packetId} dependency prefix is not yet supported by the acyclic core verifier`);
  }
  const selected: Phase10C0VS6VerifiedPublishedPacket[] = [];
  for (const packetId of requiredCoreIds) {
    const reproofs = semanticReproofs.filter((entry) => entry.packetId === packetId);
    if ((packetId === "c0v-moving-publish") !== (reproofs.length === 1)) {
      fail(`${packetId} semantic reproof nullability differs from its packet class`);
    }
  ...
  const selections: readonly Phase10C0VS6DependencyDispositionSelection[] = Object.freeze(
    selected.map((entry) => Object.freeze({
      packetId: entry.packet.packetId,
  ...
  assertSelectedPrefixClosedWorld(authority, current.packet, selected);
...
/**
 * Live-current-preflight deep prefix verifier including the historical moving-publication packet.
 * Its semantic reproof is derived inside this module from raw historical candidate bytes and the
 * cycle-free semantic leaf; no caller-authored reproof token enters the public seam.
 */
export function independentlyReopenPhase10C0VS6VerifiedPublishedDependencies(
  ...
  const current = derivePhase10C0VS6RetainedRuntimeAuthority(input);
  const shallow = phase10C0VS6ReopenPublishedDependencies(input);
  const reproofs = current.packet.boundDependencyPacketIds.includes("c0v-moving-publish")
    ? Object.freeze([independentlyReproveMovingPublicationPrefix(input, current, shallow)])
    : Object.freeze([]);
  return verifyProduceDependencyPrefix(input.repositoryRoot, Object.freeze({
    ...current,
    packetProtocolIdentity: input.packetProtocolIdentity,
 ...
 * current preflight only in memory while independently rederiving moving-publication semantics.
 */
export function independentlyVerifyPhase10C0VS6ObservedPublishedDependencyPrefix(
  ...
  const reproofs = packet.boundDependencyPacketIds.includes("c0v-moving-publish")
    ? Object.freeze([independentlyReproveMovingPublicationPrefix(input, current, shallow)])
    : Object.freeze([]);
  return verifyProduceDependencyPrefix(root.path, current, shallow, reproofs);
}
