...
function packetCallables(packetId) {
  ...
  return [...ids].sort().map((callableId) => {
    ...
    return { callableId, role, resolution: "planned", modulePath: location[0], exportName: location[1], identity: null, producedOutputIds, invokedCheckIds, evaluatedCheckIds, executedNegativeControlIds };
  });
}
...
function launcherBoundaryContractFor(packetId, cataloguePacket, subroutes, candidateRosters) {
  const attemptId = `${packetId}-20260822-v1`;
  const paths = allowedPaths(packetId);
  const transitions = paths.map((finalPath) => ({ finalPath, stagingPath:`${finalPath}.stage-${attemptId}` }));
  const pair = (finalPath) => {
    const transition = transitions.find((entry) => entry.finalPath === finalPath);
    if (!transition) throw new Error(`missing launcher publication transition ${finalPath}`);
    return [
      { kind:"boundary-recheck", boundaryId:`publication-stage:${transition.stagingPath}` },
      { kind:"boundary-recheck", boundaryId:`publication-install:${transition.finalPath}` },
    ];
  };
  const verificationPaths = paths.filter((entry) => entry.endsWith(`/${cataloguePacket.verificationFilename}`));
  if (verificationPaths.length !== 1) throw new Error(`missing exact launcher verification path ${packetId}`);
  const runRosters = subroutes.map((subroute) => {
    const filenames = candidateRosters[subroute.subrouteId];
    if (!filenames) throw new Error(`missing launcher candidate roster ${subroute.subrouteId}`);
    const candidatePaths = filenames.map((filename) => {
      const matches = paths.filter((entry) => entry.split("/").at(-1) === filename);
      if (matches.length !== 1) throw new Error(`launcher candidate ${filename} is not one final path`);
      return matches[0];
    }).sort();
    const requests = [
      { kind:"initial-auth", boundaryId:"entry-before-arguments" },
      ...pair(cataloguePacket.preflightReceiptPath),
      { kind:"boundary-recheck", boundaryId:`terminal-candidate:${cataloguePacket.attemptRoot}/${attemptId}/terminal-success-candidate.json` },
      ...candidatePaths.flatMap(pair),
    ];
    const verificationOutputs = subroute.requiredOutputIds.filter((outputId) => outputId.endsWith("-verification"));
    if (verificationOutputs.length > 1) throw new Error(`repeated launcher verification output ${subroute.subrouteId}`);
    if (verificationOutputs.length === 1) requests.push(...pair(verificationPaths[0]));
    requests.push(
      ...pair(cataloguePacket.terminalReceiptPath),
      { kind:"boundary-recheck", boundaryId:`cleanup-packet-lock:${cataloguePacket.lockPath}` },
      { kind:"boundary-recheck", boundaryId:`cleanup-package-lock:${catalogue.packageLockPath}` },
      { kind:"release-output", boundaryId:"release-output" },
    );
    return { subrouteId:subroute.subrouteId, requests };
  });
  return {
    schema:"phase10-c0v-s6-launcher-boundary-contract-v1",
    requestExactFields:["kind","boundaryId"],
    checkRequests:[
      { kind:"initial-auth", boundaryId:"entry-before-arguments" },
      { kind:"release-output", boundaryId:"release-output" },
    ],
    runRosters,
    derivationRule:"common-preflight-stage-install-then-terminal-candidate-then-selected-path-ordered-stage-install-then-optional-verification-stage-install-then-terminal-stage-install-then-path-bound-cleanup-and-release",
    failureRule:"failure-is-strict-nonrelease-prefix-success-is-complete-roster-with-release-last",
  };
}
...
for (const packetId of packetIds) {
  ...
  const protocol = {
    ...
    launcherBoundaryContract:launcherBoundaryContractFor(packetId,cat,packetTerminalSubroutes,candidateRosters),
