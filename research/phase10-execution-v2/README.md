...
from the launcher's exact nine-row clean-environment authority, and the worker independently
requires full equality to that same block before dispatch. These checks reject only state still
visible when each Node entrypoint begins;
a preload or loader that executes first and erases its own environment/`execArgv` trace is not
excluded by the in-process observation. A separately designed and frozen external pre-Node
launcher must establish that boundary before implementation freeze. Its command/host trust and
all sixteen command authorities are unresolved here, so execution readiness remains blocked. The
catalogue now strict-parses the launcher design state, but no launcher source, compiler/build
receipt, executable identity, command rewrite, or run authority exists. The parent now constructs
the worker environment solely from the catalogue's exact nine-row clean block, and the worker
independently requires its complete process environment to equal that block; no ambient value is
cloned.

The prospective launcher channel remains open through the whole action. Requests and responses
are one-to-one, zero-based, and exactly one of `initial-auth`, `boundary-recheck`, or
`release-output`; every request uses a fresh 32-byte challenge. Each packet protocol carries one
exact `launcherBoundaryContract`: `check` is initial-auth then release, while every terminal
subroute has its own exact run roster. All run rosters share preflight stage/install as their
common prefix, then bind the direct terminal-candidate publication, each selected path-ordered
stage/install pair, optional verification stage/install, terminal-receipt stage/install, the
path-bound packet/package cleanup checks, and release last. The native state machine must accept
only the complete selected roster on success; a failure may end only at a strict non-release
prefix.
The inherited launcher-to-Node response read end is OS-nonblocking before child creation. Win32
uses `CreatePipe` and checks `SetNamedPipeHandleState(..., PIPE_NOWAIT, ...)` on that read handle
before `CreateProcess`; POSIX checks `fcntl(..., O_NONBLOCK)` before `exec`. Node polls using
parent-owned `hrtime`; only `EAGAIN` or `EWOULDBLOCK` means no bytes, while a zero-byte read is EOF
and every other read error fail-stops. Before every request, any queued unsolicited byte or EOF
rejects. After a canonical nonrelease response, Node drains the currently queued response bytes:
any nonempty drain or EOF rejects, and a byte arriving later rejects before the next boundary.
Each request-write-to-response interval, and launcher child-create-to-initial-request interval, has
an exact 30,000 ms maximum. Release success additionally requires exact EOF after the canonical
response with no intervening byte under that same request-write-to-response-plus-EOF deadline;
equality passes and one millisecond more fail-stops.
The launcher self-opens and hashes its image before child creation and each response but never
embeds its own final SHA-256; Node independently reopens and hashes the externally resolved
catalogue identity. Only after writing the final release response does the launcher close child
stdin; Node must observe that exact EOF before it may authorize output release. The launcher then
raw-proxies output using a fixed 65,536-byte drain-before-next-read buffer with no aggregate
launcher accumulation. A failed action may end with only a strict control-message prefix and no
executor output.

## Argument vectors — execution disabled

The sixteen direct-Node lines below are the currently frozen child argument vectors, retained only
so their exact packet/attempt scope remains inspectable. They are **retired as authorizing
commands**: `directNodeRule` rejects them even if the arguments and environment are otherwise
exact. No S6 command is executable while the native launcher source, compiler/build receipt,
binary identity, authenticated parent implementation, and launcher-prefixed command rewrite are
absent.

Once that separate resolution lands, `check` is read-only and acquires no lock or creates no
attempt/output. It validates only the exact
arguments and compiled catalogue/protocol/registry/attempt authority; it explicitly does not
observe an authorizing preflight, resources, or mutable dependency state and cannot authorize or
resume `run`. `run` acquires the compiled package lock and then the compiled packet lock before
reading the catalogue, protocol, registry, repository state, dependencies, or resources. Argument
order is exact; extra flags reject. These semantics describe the prospective launcher-mediated
commands, not permission to run the direct-Node lines below.
