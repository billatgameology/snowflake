...
separate verifier agents—described by the historical workflow as independent—then reportedly
re-opened the cited pages for the 381 items flagged high-consequence and checked, for each, that the
quoted sentence actually appears there and that the value is transcribed correctly. The producer's
record says **381 of 381 passed; none failed**; this correction has not independently reproduced that
count. A synthesis agent then assembled this document.
...
**Historical Rule 10 limit.** The surviving record does not identify the extraction or verifier
models, state whether they shared developer context, or preserve a per-item execution transcript
from which reviewer independence can now be established. The 381/381 result is the historical
workflow's reported count, not a fresh reproduction in this correction session. The personal
source checks listed next are the only independently named re-executions retained here; beyond
them and the reported 381-item scope, §9D is the explicit unchecked set. Treat this provenance as
incomplete under the current Rule 10 until a future review records model, context, reruns, and
...
**Status correction (2026-08-01).** Some extracted M1 forms entered the historical arm-2 protocol,
and both 204-row CAK and M1 sweeps ran. The corrected working-tree parameter table now also transcribes
the later broad forms and FACET table. ADR 0040 accepted that corrected mapping under a separately
named current-file hash; it does not rewrite the historical hashes or upgrade either sweep. Charter §3.2
Phase 6 item 1 still applies: a post-freeze parameter/protocol edit requires a logged ADR and full
rerun before replacement-gate use.
...
separate extraction agent, with the high-consequence items reportedly re-opened and checked against
the source pages by separate verifier agents in the historical workflow. Every number below carries
the verbatim sentence or table row it
...
## 1. What conflicted with the historical project baseline

The `Our value` column records the historical CAK/project state at extraction time unless a row
explicitly names M1 or a later dated correction. It is not a claim about every parameter set exposed
by current code or the proposed replacement table.
...
| # | Topic | Historical/project value at extraction | Their value | Paper | Page | Verbatim |
|---|---|---|---|---|---|---|
| H1 | Source-model claim: broad-facet kinetics alone do not make columns in the -3 to -10 C band | The named historical 204-point CAK arm measured zero columns in -3.3..-9.9 C; this is not a universal bound | Broad-facet behaviour alone explains only plates at T > -3 C and columns at T < -30 C; the basal SDAK dip is what yields columns near -4 C | CM9 | 11 | "The broad-facet behavior alone is sufficient to explain the formation of plates at T>-3C and columns at T<-30C, as seen from the plot of sigma_0,basal and sigma_0,prism in Figure 1. The fact that A_prism < 1 at high temperatures complicates the picture, but only to a minor extent. Adding the basal SDAK dip yields strong columnar growth near -4 C, while the prism SDAK dip produces strong platelike growth near -14 C." |
| H2 | Source-model claim: SDAK supplies both missing habits | The historical CAK arm omitted SDAK; CAK→M1 is confounded, and a matched M1/no-dip arm is required to isolate the implemented dip factors' effect on this solver under a frozen configuration. Even that intervention cannot establish physical SDAK causality or necessity in nature | SDAK on basal surfaces is stated NECESSARY for columns at -4 to -6 C; SDAK on prism surfaces RESPONSIBLE for plates at -11 to -18 C | CM10 | 3 | "More generally, the CAK model suggests that the SDAK effect on basal surfaces is necessary to produce columnar morphologies between -4 C to -6 C, while a similar phenomenon on prism surfaces is responsible for platelike forms between -11 C to -18 C. In both cases, the attachment kinetics is markedly different on narrow and broad facet surfaces." |
| H3 | Which sigma_0 curves apply to a narrow-faceted crystal | Historical CAK used one broad-facet sigma_0 curve pair for all crystals | Two distinct curve pairs exist; the SDAK-dipped pair "apply only to narrow facet structures", and it is that pair that reproduces the Nakaya morphologies | CM10 | 5 | "Figure 3: The CAK model incorporates the SDAK effect to add "SDAK dips" to the sigma_0 plots in Figure 1, yielding the two curves shown here, which apply only to narrow facet structures. Together, these curves explain the platelike/columnar morphologies in Nakaya diagram as being strongly shaped by edge-dependent surface diffusion [2021Lib]." |
| H4 | SDAK enters as a parameter substitution keyed on facet structure | Historical CAK made sigma_0 depend only on facet type and temperature | Replace sigma_0,x by sigma_0,x,SDAK whenever the surface is not broadly faceted | CM9 | 4 | "For reasons that are only partly understood at present [2019Lib1], it seems to work reasonably well to approximate the resulting growth behavior by replacing sigma_0,x by sigma_0,x,SDAK whenever the surface in question does not exhibit a broadly faceted structure." |
| H5 | Existence and location of a dip in sigma_0,basal | Historical CAK had no basal dip; sigma_0,basal was monotonic in (Tm - T); A_basal identically 1 | A dip in sigma_0,basal near -4 C is postulated and stated confirmed by growth-rate measurement | CM10 | 4 | "The CAK model additionally postulates a corresponding dip in sigma_0,basal near -4 C, and the combined effect yields columnar from -4 C to -6 C and platelike growth from -11 C to -18 C, as seen in the Nakaya diagram. Growth-rate measurements confirm the existence of SDAK dips at both -14 C [2020Lib1] and -4 C [2020Lib2]" |
| H6 | Model-inferred magnitude of the SDAK dip in sigma_0,prism at -14 C | Historical CAK had no dip (factor 1.0) | The source's witness-surface/model-conditioned inversion puts the narrow-facet value at about one-tenth of its broad-facet value; surface supersaturation was not independently measured | CM10 | 9 | "A factor-of-two reduction in the effective sigma_0,prism is perhaps too small to reproduce the actual SDAK dip at -14 C, where sigma_0,prism is about one-tenth of its broad-facet value [2020Lib1]." |
| H7 | Facet-width dependence of alpha_prism | Historical CAK depended on facet type, T, and sigma_surf only, with no width dependence | alpha_prism depends "quite strongly on facet width w" in addition to T and sigma_surf | CM8 | 12 | "In this picture, the value of alpha_prism depends on temperature T, surface supersaturation sigma_surf, and also quite strongly on facet width w. In Figure 12, it is the large change in w that brings about the rapid change in alpha_prism,SDAK observed." |
| H8 | Facet-width dependence of alpha_prism (same 2020 source lineage restated) | as above | TRIG restates the CM8 inference that alpha_prism at -14 C increases roughly 100-fold as thin plate edges develop; this is not an independent evidence route | TRIG | 12 | "Nevertheless, the measurements in [2020Lib1] show that alpha_prism at -14 C increases roughly 100-fold as thin edges of platelike crystals develop, so it would not be surprising to see a 60% increase in alpha_prism between the short and long facets in Figure 15." |
...
| Source-model inversion of basal growth observations, centered near -4 C | centered near -4 C | CM9 | 7 | "Measurements of the basal SDAK parameter sigma_0,basal,SDAK as a function of temperature, revealing a substantial "SDAK dip" centered near -4 C." |
...
| Source claim that model-conditioned growth-rate analyses support dips at | -14 C and -4 C | CM10 | 4 | "Growth-rate measurements confirm the existence of SDAK dips at both -14 C [2020Lib1] and -4 C [2020Lib2], providing a compelling validation of the overall model structure." |
...
**The paper states its own mechanism falls short of the source-inferred, model-conditioned effective
dip depth by roughly a factor of five; surface supersaturation was not independently measured:**
...
### 5C. Uncertainty on the 206-observation candidate corpus (in-sample; `passEligible=false`)

These observations informed TAX2's M1/P3 construction and predominantly use electric-needle growth.
They are a valuable source index and blind-reconnaissance corpus, but are not a held-out Phase 6
validation target under the current source lock.
...
|---|---|---|---|---|
| CM8 | 18 | 15 | x = -Temp (C), log, 5 to 40; y = sigma_0,basal / sigma_0,prism (percent), log, ~0.1 to 20 | A model-conditioned inversion for sigma_0,prism,SDAK(T), with plotted error bars, spanning roughly -8 C to -30 C; `sigma_surf` is inferred and only the coefficient ratio is experimentally constrained (§5B). Two fitted points are stated numerically in text (0.85% at -10 C, 6.6% at -25 C). |
| CM9 | 5 | 7 | x = -Temperature (C), log, 2 to 10; y = sigma_0 (percent), log, 0.01 to 1 | A model-conditioned inversion for sigma_0,basal,SDAK(T) near -4 C, with horizontal and vertical error bars, on the same axes as the broad-facet curves. No tabulated series is printed in CM9; TAX2 later prints an approximate M1 closed form. |
| CM9 | 6 (bottom panel) | 8 | x = -Temperature (C), log, 2 to 9; y = v_basal / v_prism, linear, 0 to 10, with a reference line at 1 | An observed axial/transverse growth-velocity ratio versus temperature in the source's needle experiment. It is more direct than the inferred coefficient curves, but its seed/geometry and M1 provenance make it an in-sample morphology reference rather than a held-out analogue to the project's 3-D aspect-ratio score. |
| CM8 | 12 | 11 | x = sigma_surf (percent), log, 0.3 to 3; y = alpha_prism, alpha_basal, log, ~0.004 to 1 | Model-conditioned attachment-coefficient points at -14 C, annotated by `sigma_far`; `sigma_surf` is witness-surface inferred and the experiment constrains a ratio, not two independently measured coefficient functions (§5B). Useful for auditing the source inversion, not as direct measurement of SDAK magnitude. |
| CM8 | 4 | 5 | Two stacked panels, x = (Tm - T) (C), log, 1 to 50; y = sigma_0,basal (percent) and sigma_0,prism (percent), log, ~0.05 to 20 | The CAK source model's predicted narrow-facet dips. CM8 prints no closed form for these original dotted curves, so digitization is required to reproduce that particular figure; TAX2 later prints the separate approximate M1 forms. |
...
| CM6 | 8 | 12 | x = R, H (microns), 0 to 7; y = Number, 0 to ~21; two hatched histograms | A measured (R, H) distribution for -5 C simple prisms at sigma_inf ~ 0.2%. It supplies a candidate aspect-ratio distribution, but seed, ensemble selection, geometry, and endpoint mapping must be registered before comparison with the project's score. The caption states columnar crystals were entirely absent. |
...
| CM8 | 14 | 13 | y = R, H (microns), 0 to ~60; x = Growth Time (s), 0 to 250 | A -25 C columnar R(t)/H(t) series from the same source lineage, with a cone-with-fins sketch unlike a simple hexagonal prism. It is an in-sample quantitative diagnostic candidate, not a geometry-matched gate target. |
| CM8 | 19 | 16 | x = -Temperature (C), linear, ~11.7 to 16.3; y = Velocity (um/sec), ~0.3 to 1.25 | Observed radial plate growth velocity versus temperature at fixed sigma_far = 22%. Its peak is −14.15 +/- 0.15 C; that constrains the growth-rate peak under this setup, not by itself the minimum of a model-inferred SDAK dip. |
...
| TAX1 | 11c | 7 | x = time (seconds), -40 to ~110; y = radius (microns), 0 to ~68; two series (plate radius, needle radius) with model curves | The quantitative growth-rate data in TAX1: plate radius and needle radius versus time for plate-on-needle growth with a matched source-model curve. It is an in-sample, needle-geometry diagnostic candidate, not a held-out validation target. |
...
| TAX1 | 6, 7, 8 | 4, 5 | Various (T, supersaturation) morphology diagrams reproduced from [1954Nak], [1958Hal], [1961Kob], [1990Yok] | Multiple historical habit-regime diagrams reproduced by TAX1, including Figure 7 middle ([1961Kob]) with a numerical c/a aspect ratio and Figure 8's four bands. They are separate literature lineages but not automatically independent held-out evidence for M1, whose construction is Nakaya-informed. |
| FACET | 2 and 3 | 10 | x = Prism growth velocity v_facet (nm/sec), log, 0.1 to 1000; y = R/r_corner, log; black curves = latent heating ignored, red = included | R/r_corner vs v_facet from Libbrecht's own model, with/without latent heating at R = 20 and 500 um. This is a model-to-model transcription/implementation check, not physical validation of the heating term. |
| CM10 | 1 / CM9 | 1 / 2 | Two stacked panels, x = (Tm - T) (C), log, 1 to 50; y = sigma_0 (percent) log and A log 0.1-1 | The broad-facet sigma_0(T) and A(T) source-fit figure family republished within the same author/source lineage. Re-digitizing can cross-check transcription consistency across printings, not independently reproduce a measurement, and confirms the basal A line is drawn flat at 1. |
...
2. **No figure was digitized.** Section 6 lists 27 digitizable figures. Every one of them is listed
   ...
   model-conditioned SDAK inversions - CM8 Figure 18 (sigma_0,prism,SDAK(T) from about -8 C to -30 C)
   and CM9 Figure 5 (sigma_0,basal,SDAK(T) near -4 C) - are plot-only in those two measurement papers.
   Their prose states 0.85% at -10 C and 6.6% at -25 C (CM8 p.14, p.13), plus CM10's ratio “about
   one-tenth of its broad-facet value” at -14 C. Exact fitted widths and the original plotted series
   are therefore unavailable as tabulated numbers from that subset; TAX2 separately prints an
   approximate M1 parameterization.

3. **No exact facet-width closure was found.** CM10 p.10 states that the complex surface behavior
   cannot presently be calculated from first principles, and CM6 p.4 says the facet-width dependence
   is not well defined. TAX2 nevertheless prints approximate temperature-only M1 dip forms for an
   everywhere-narrow-facet model. Those forms are not an interpolation law between broad- and
   narrow-facet endpoints and do not supply the missing local facet-width closure.
