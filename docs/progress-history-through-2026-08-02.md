# Progress history through 2026-08-02

> **Historical snapshot — not current authority.** The current project state is
> [PROGRESS.md](PROGRESS.md), with the exact cold-resume point in [HANDOFF.md](HANDOFF.md).
> Read this archive only when a current record links to historical detail.

This file preserves the complete pre-compaction `docs/PROGRESS.md` body byte-for-byte, including
its mixed line endings. Snapshot identity: 191,859 raw bytes, SHA-256
`2550319a3ac5d528c111875242419de91d2ed9b34f245f7a0364ede8b323f955`; LF-normalized
identity: 190,074 bytes, SHA-256
`9f7ee2ad0a7773740b8aff111b16aad236fb9555f7ae0cd861714681103b4a9d`.

Historical citations of the form `docs/PROGRESS.md:<line>` refer to the body below, counting its
first `# Progress` line as line 1, not to this archive file's displayed line number. Relative links
remain resolvable because the archive stays beside the original file in `docs/`.

<!-- BEGIN EXACT PRE-COMPACTION PROGRESS BODY -->
...
- **Phase 6 is ACTIVE AND INCOMPLETE (corrected through 2026-08-02).** The two historical 204-row artifacts
  ...
  earlier WP0 correction review was partial; later complete-diff/ADR 0040 audits found additional
  non-education defects, which now have a fresh 0-blocker / 0-substantive-should-fix non-author
  verdict. Accepted ADR 0040 now governs current charter v1.19. On 2026-08-02 the maker froze all
  further education-site work until Phase 6 is complete: the landing page warns that the course may
  lag the current science and is not the authoritative status/validation record, while the existing
  44-file education snapshot is preserved in local, unpushed checkpoint `60e3f3f` (6,404 insertions /
  2,904 deletions relative to its parent). Education content, figures, visual QA,
  verifier expansion and acceptance review are no longer on the Phase 6 critical path; they return
  only for post-Phase-6 reconciliation. WP1 now
  ...
  campaign per physical-size stratum (`612 × S × D × Z` overall); no valid D6h/monopole theorem currently reduces it,
  ...
  executed rows are unchanged. Exact root `npm test` on the complete committed `8dc7a60` unit exited 0 in
  731.9 seconds: Rule 7 clean over 426 files, both TypeScript projects green, and 78 files / 1,383
  tests passed (Vitest 721.88 seconds). That unit is committed locally as `8dc7a60`.

  A fresh non-author R15 audit then disproved the summary claim that every live propagation of the
  withdrawn `sigma_0`-crossing theorem had been corrected. The first pass named four files; the
  expanded pass found additional obsolete roots/labels/universal conclusions in the superseded Phase
  6 plan and `research/phase6-convergence.md`, plus the solver spec, stretch/publication note, video
  explainer and papers-extract table. Those non-education surfaces replace obsolete roots and
  theorem/causal/priority language with measured/source-scoped claims. Separate code/test,
  dataset/report and recordkeeping reviews initially closed, but later complete-diff/ADR 0040 audits
  found additional status, hash, unit, inventory, source-provenance, charter-quote and causal-scope
  defects. Those non-education repairs now have a fresh clean non-author verdict. The reclaimed
  education repair is now integrated and materially expanded. The current focused verifier exits 0
  with **153/153 named negative controls** in both public-source and offline-source modes. Its
  authority path independently parses the 18-file / 974,627-byte evidence manifest,
  all three 204-row sweep/report pairs, every M1 report/config identity field, both 448-entry
  fingerprints, the held-out candidate lock, pressure context and tracked state records. It also
  accepts a coherently named invalid protocol-scope row while removing it from both denominators and
  rejects coherent exclusion-reason/identity misbinding. The review found that Tier 2 arm64 raw logs
  and exit records were never tracked and are unavailable; all current records now call the four
  matching rows a non-rederivable historical report, and lesson A1 records this repeated preservation
  failure. A targeted Chromium visual QA passed 12 profiles / 24 screenshots after repairs to the
  chapter 13 mobile chart, chapter 28 exact 204-point touch reader and chapter 29 chip wrapping
  (`out/education-visual-qa/report.json`). The final arm-2/protocol/SDAK set passes 3 files / 101
  tests, and the broader protocol/sweep/evidence/cross-platform set passes 5 files / 121 tests.
  Exact charter quote coverage remains clean at 22/22, but the integrated acceptance review reopened
  the education copy after finding final provenance-scope and mapping-attribution defects in chapter
  16. Those defects are repaired and now have an enforcing evidence-integrity test with six named
  negative controls. The current focused public Part Two verifier passes 153/153 controls. The prior
  complete-course result (213 checks over 33 pages / 179 roots, including 198 profile loads per mode
  and 165/165 controls) predates these final bytes and is superseded until rerun. Exact current-tree
  `npm test`, the complete public/offline verifier, final non-author acceptance and the local commit
  remain WP0 landing steps; none of these checks is R15 evidence.

  The corrected `docs/libbrecht-parameters.md` now classifies analytic/contextual relations,
  source fits and reconstructions without promoting them to direct measurements. Accepted ADR 0040
  will give the accepted current authoritative LF-normalized revision its own content pin. The prior
  48,359-byte / `be00e00b…` candidate is stale after the final scoped-provenance repair and must be
  recomputed and propagated only after the ongoing independent audit closes. Historical registered hash
  `276494f69682adb2b071c2e2683a98281aef17b3558b4efa6301ceaf11dfa741` is deliberately unchanged
  inside arm 1's legacy values manifest. Arm 2's historical M1 values
  manifest never carried a table digest and retains its exact independent `13e678d5…` identity rather
  than gaining a post-result field. The R15 protocol does not freeze here;
  it remains blocked on WP1/WP2. The earlier exact root `npm.cmd test` exited 0 in
  723.7 seconds: Rule 7 clean over 415 files, both TypeScript projects green, and Vitest passed 78
  files / 1,394 tests in 713.98 seconds; it predates the new enforcing test and final table copy, so
  a current-tree rerun is mandatory before landing.

  The mixed root `tmp/` cache was inventoried on 2026-08-01. Nine durable third-party source files
  (23,215,628 bytes) were moved to ignored `research/` paths and recorded with stable identifiers,
  byte counts, page/member counts and SHA-256 in `research/phase6-source-currency.md`. The remaining
  derivative renders, parse cache and one-off extraction dependencies were preserved intact under
  ignored `research/tmp/`; they remain transient/provenance-incomplete and are not evidence. The five-file source-lock verifier
  re-executed from the new paths and returned `files=5 members=21 maxGap=0.9451000000000249s
  passEligible=false`. No generated scientific result was found in the former root `tmp/`, so nothing was promoted
  into `evidence/`.
  ...
  arm-2 reader and the flip census. The earlier focused checkpoint/timeline/LK/arm-2 run passed **5
  files / 121 tests**, and the earlier broader Phase 6 group passed 106/107 with only the deliberate
  corrected-table versus historical-table-pin mismatch. Those suite counts are superseded pending a
  current-tree rerun. The latest direct canonical recomputation matches the
  candidate constants: arm 1 values/justification/combined are `879e069f…`, `13911b85…`,
  `6f35c2bc…`; arm 2 values/justification/combined are `13e678d5…`, `709646e5…`, `21b16a7b…`.
  Values hashes did not move, and every intermediate prose/combined hash remains in revision history.
  The final current-table re-pin, exact root `npm.cmd test`, scoped non-education independent verdict
  and local landing commit are the remaining WP0 actions. The complete education verifier and
  education acceptance review are explicitly deferred until Phase 6 is complete.

  Decision [0039](decisions/0039-cycle-boundary-lk-resume-checkpoints.md) is again **proposed**. A
  non-author design audit found that the contemplated 744³ fields alone occupy 7,001,123,328 bytes,
  so a monolithic checkpoint or constructor-then-copy restore is invalid. The design streams exact
  binary64/u32 state, adopts decoded fields without a second full copy, preserves the monopole lag and
  boundary order, and binds immutable recovery generations. A later implementation-readiness audit
  superseded the prior clean verdict: it proved that unattached boundary `f=1` is reachable, found the
  exact run-spec/manifest/policy/cadence contracts cannot be accepted before WP3 freezes their inputs,
  and caught complete-chain retention wording inconsistent with the recovery verifier. The proposed
  ADR now accepts the reachable state, requires retention to genesis and defers runner-dependent
  details. The exact 1,095-line / 80,760-byte pre-review-record candidate at
  `b9bfec3708b8ef04feed040a8c02d1cda54187dee90f990bb49d74355d33b69f` received a fresh clean
  non-author core-design verdict: 0 blockers / 0 core should-fixes. Implementation still follows WP0;
  crash tests, direct/resumed differentials, a separate code/evidence review, and the WP3-dependent
  runner freeze/review remain open.
...
all five now exist; `app` landed in Phase 3 and the tracked `solver-gpu` package landed under the
completed Phase 5 Windows/Chromium/D3D12 gate).
...
- **[libbrecht-parameters.md](libbrecht-parameters.md)** — σ₀(T), A(T), v_kin(T), D(T,P).
  **Extracted 2026-07-15** (first pass, not yet frozen): every entry cited with pages,
  ...
  own header now scopes the original extraction limit: σ₀/A curves were figure-only in the
  monograph/1910 source pair audited on 2026-07-15 (digitized anchors, labeled, ±25%); later
  2009/2023 primary papers print forms. No D(T) law exists in that original source (gap recorded).
...
the parameter table and validation protocol **freeze before the first sweep** (pre-registration),
  and any post-freeze edit requires a logged ADR and invalidates prior sweep evidence for a
  replacement gate; historical executed measurements remain preserved at their measured-only scope.
Accepted charter v1.19 and ADR 0040 correct the former v1.18 overstatement: authorized changes are
auditable and undisclosed drift is detectable, while misconduct is not made impossible.
...
Continuation—the index previously stopped at 0021 even though later records are authoritative:

- Phase 5/early Phase 6: [0022](decisions/0022-phase5-observed-negative-control-ownership.md)
  negative-control ownership; [0023](decisions/0023-d6h-equivariant-opposing-vapor-mean.md) D6h
  reduction; [0024](decisions/0024-monopole-matched-far-field.md) monopole far field;
  [0025](decisions/0025-phase6-agreement-scoring-rule.md) scoring;
  [0026](decisions/0026-phase6-grid-extrapolation-operator.md) extrapolation;
  [0027](decisions/0027-charter-far-field-amendment.md) charter repair;
  [0028](decisions/0028-parameter-table-erratum-exponent.md) parameter erratum; and
  [0029](decisions/0029-phase7-view-profiles-and-designer-compiler.md) Phase 7 profiles are accepted.
- Phase 6 freeze/evidence history: [0030](decisions/0030-phase6-sdak-arm.md) was superseded before
  acceptance; [0031](decisions/0031-phase6-paramset-freeze-row.md),
  [0032](decisions/0032-phase6-crossplatform-fixture-selection.md),
  [0033](decisions/0033-phase6-values-justification-hash-split.md),
  [0034](decisions/0034-phase6-e1-justification-correction.md),
  [0035](decisions/0035-phase6-run-outcome-and-input-pins.md),
  [0036](decisions/0036-phase6-sdak-arm-pre-registrations.md),
  [0037](decisions/0037-phase6-domain-spot-check-failure.md), and
  [0038](decisions/0038-evidence-tree-is-tracked.md) are accepted records, with the interpretation
  and scheduling supersessions stated in their own headers.
- Current candidate: [0039](decisions/0039-cycle-boundary-lk-resume-checkpoints.md) remains
  **proposed**. Its reviewed protocol-independent core may be implemented after WP0; its
  runner/evidence layer remains WP3-deferred.
- [0040](decisions/0040-correct-phase6-coefficient-and-sdak-provenance.md) is **accepted** and
  governs current charter v1.19 plus the separately named current parameter-table content pin. It
  does not freeze the still-blocked R15 protocol.
...
- [phase-6-science-first-completion.md](plans/phase-6-science-first-completion.md) — active. The
  maker selected execution of O1b rather than narrowing the charter. WP0's evidence/interpretation
  correction, accepted ADR 0040/current charter v1.19, education repair and combined propagation
  review are clean; exact root `npm.cmd test` passes and only the local landing commit remains. WP1's candidate-source lock is
  complete and verified, while the selected validation
  ...
  `612 × S × D` domain rows per nested size trajectory under the current whole-grid rule; its
  three-spacing/three-domain baseline is 5,508, and the cross-term applies if both axes expand. A
  blind, pre-registered TAX2 panel segmentation can derive
  terminal pixel span × field-of-view size, but its electric-needle geometry must be implemented or
  bounded and it remains in-sample for M1. Nelson 1998 and Bacon et al. 2003 exact public bytes plus
  the official Bailey–Hallett 2002 precursor were audited; the indexed 2004 publisher rendering was
  reviewed, but no 2004 PDF/hash was available. All three target families were rejected for the
  current solver because of support/initial-state, coupled-condition, inferred-field, sublimation, or
  thermal mismatches.
  Held-out targets remain blocked until
  their geometry, source currency, independence and uncertainty are apples-to-apples.
...
For any unattended long task, update this resume point and `docs/HANDOFF.md` before launch, run
only from a clean immutable source snapshot, write labeled live/error/exit logs and durable science
outputs under `evidence/`, and send user-facing status at most once per hour unless the maker asks.

1. Finish the non-education exact-metrology/provenance repair, recompute the separately named
   current-table pin, obtain the scoped non-author verdict, run the applicable focused checks and
   exact root `npm.cmd test`, then commit the reviewed WP0/ADR 0040 unit locally after the final diff
   check. Historical parameter/protocol hashes and legacy manifests remain unchanged. Preserve the
   frozen education work-in-progress and its landing-page warning; do not repair Chapter 16, rerun
   visual QA, or extend/rerun the education verifier until Phase 6 is complete. Do **not** freeze R15
   here; step 5 remains blocked on WP1/WP2. The prior candidate-source/prepared-kinetics unit is
   already committed at `8dc7a60`.
2. After that commit, implement and verify proposed ADR 0039's protocol-independent streamed v3,
   legacy byte fixtures, field-adopting restore and direct/checkpoint-every-cycle/multiply-resumed
   differentials.
3. Defer immutable R15 generation
   publication, run-spec/manifest bindings, trace-derived campaign state and crash injection until
   WP3 freezes their exact schemas and policies. V3 remains mandatory before the first R15 production
   row; cold-start-only reconnaissance remains explicitly non-transferable.
4. Continue the Yamashita/pressure lineage and pre-register a blind TAX2 segmentation/uncertainty
   operator before deriving any physical
   terminal sizes. Do not inspect model output during extraction; TAX2 remains in-sample for M1 and
   requires source-matched electric-needle geometry. Exact-byte audits rejected Nelson 1998, Bacon et
   al. 2003 and the official Bailey–Hallett 2002 precursor; the indexed 2004 publisher rendering was
   reviewed without a PDF/hash claim. Those target families remain current-model mismatches; continue
   the pressure/Yamashita lineage search without treating their 100–300 µm ranges as a size
   prescription.
5. Write and independently review the replacement ADR/protocol only after WP1 supplies admissible
   held-out targets and WP2 supplies a passing
   ...
   If the source search ends without targets, Phase 6 remains incomplete; a source-limited result is
   reportable but cannot substitute for the charter obligation or permit a gate pass.
6. Do not launch R15, GPU, matched-ablation or held-out production rows until the immutable inputs,
...
| | result |
|---|---|
| **Cross-platform control** (B11; record limit added 2026-08-01) | tier 1 **differs** (`2a9f64b3` vs `3662b9e2`) from tracked bytes; a historical table reports four identical Tier 2 rows, but its raw arm64 logs/exit records were never tracked and are unavailable, so class portability is not independently rederivable |
...
size divergence was caused by the dip mechanism (both arms diverge; the bundled M1 parameterization
changes the sampled curve, but this comparison does not isolate a cause).
...
**Historical Phase 6 arm 2 (bundled M1 parameterization) ran, and both arms are reported together at
[research/phase6-two-arm-report.md](../research/phase6-two-arm-report.md) (2026-07-30).**

> **Common denominator: arm 1 3/90, arm 2 54/90**, against the withdrawn/confounded historical
> proxy forecast of 42/90, which is inadmissible as habit evidence and was not a valid pre-run habit
> prediction.
...
- **The bundled M1 arm is a TRADE, not an improvement in this extent-21 artifact.** It converts 66 neutral points to plates and pays with
...
- **At the registered extent 21, neither arm produces a single column in the Nakaya `columns`
  regime** (−3.3…−9.9 °C), 36 points each. Changing CAK to the bundled M1 parameterization moved
  **zero** of those points to `column`; it does not isolate the dip factors as the cause.
...
- **The dominant class is still "no habit":** 119 of 204 neutral in the M1 arm, down from 168.
...
**Historical discount record for 54/90:** it is in-sample because the exact dip functions were
chosen against this diagram; the prediction used an inadmissible CAK-to-M1 transfer fit and has no
habit-evidence standing; and **all five `plates-warm` agreements are at −2 °C**, the regime's only
counting temperature. The former “4.81× more distant from the column floor” interpretation is
withdrawn with that transfer fit. A separate artifact limitation remains: it was **regenerated**,
not written by its own sweep (erratum E4).

**A measured property of the 408-row extent-21 artifact, found while checking the columns claim.**
`AR = zExtent / tExtent` with `zExtent` an integer layer count, so AR is discrete. **Those 408
measurements across both arms produced 36 distinct AR values**; near the column floor the realized ladder is 1.3125, 1.4000,
...
- **The historical “~50 nm closure is avoided” claim is withdrawn.** One source example gives a
  ~50 nm controlling terrace width against Δx = 350 nm. Arm 2 runs **M1**, an everywhere-narrow
  starter approximation; that does not resolve or avoid the width scale. The width-dependent
  **M2** still requires an unimplemented closure or a separately justified finer-resolution
  alternative.
- **The bistable band −4/−5/−6 °C is excluded from the headline by name**, the same treatment
  ...
  regime. It moves the denominator only — the withdrawn/confounded historical proxy forecast's
  numerator was 42 either way; that number is inadmissible as habit evidence and was not a valid
  pre-run habit prediction.
- **The historical cold-end tier is now interpreted as a numeric-reference bracket, not an M1
  equation domain.** The prism dip cuts σ₀,prism to 5.5% of broad-facet at −15 °C and is still a
  36% reduction at −25 °C. Four same-lineage source/model-inferred references span −5…−25 °C;
  registered rows outside that bracket remain inside TAX2 Figure 1's displayed M1 domain and are
  not closed-form extrapolations beyond it.

**Superseded interpretation (decision 0040).** The historical 42/90 forecast transfers a
sigmaInfinity coefficient-ratio regression from CAK to M1; changing kinetics changes local depletion
and geometry, so it is a confounded proxy forecast and inadmissible as habit evidence. The former
15/15 “sense” score is retracted. The exact-arithmetic calculation establishes only three equal-field
coefficient-order swaps under one shared positive surface supersaturation. Habit classes come from
forward-run artifact bytes. A matched M1/no-dip arm is required to isolate the implemented dip
...
M1 is implemented in `core/src/libbrecht.ts` as a third `NucleationParamSet`. At its initial
implementation the libm digest remained `2a9f64b3`, the then-current manifests were unchanged, and bit equality for
`CAK`/`CAK_A1` asserted with `Object.is` across the domain — and it enforces TAX2 Figure 1's
source-displayed 1…50 °C model domain, which a closed form would otherwise exceed silently. Exact `npm test`
was green at that historical checkpoint; current verification is recorded in the active block above.

**How that withdrawn/confounded historical proxy forecast compared with the measurement:**
`plates-warm` forecast 4/6, measured
**5/6**; `columns` predicted 0/12 and measured **0/12**, a numerical match whose proposed reason is
not established; `plates-cold`
forecast 38/60, measured **49/60**. The inadmissible historical proxy totaled 42/90 while the
forward run measured **54/90**. The 0/12 numerical match does not establish the proxy's proposed
reason; CAK→M1 changes several kinetics choices. The forecast was
...
> **Superseded completion claim (2026-08-01; decision 0040 proposed).** The following block records
> a historical education audit against the earlier crossing correction. A later non-author review
> found live coefficient-vs-habit, centre-vs-equality, provenance, and verifier-semantics defects.
> A local handback audit found no separate owner/worktree/branch/commit, so current propagation is
> being repaired in this session. Do not execute the historical
> cherry-pick action below or treat its verifier result as current closure.

**Historical record: the independent audit and remediation of the complete education course was complete under
...
**Historical next action, superseded — do not execute as the current resume point:** review the complete isolated branch delta with
...
> **Superseded interpretation (2026-08-01, decision 0040 proposed).** The historical education
> verification below tested the earlier `sigma_0`-crossing correction, not the later distinction
> between input-function/equal-field coefficient diagnostics and forward-run habit. A non-author
> audit found live defects in `FIGURES.md`, the shared widget, chapters 12/13/17/28/29, glossary, index and Part
> Two oracle. The reclaimed education repair and its verifier rerun remain critical dependencies;
> the old “re-verified” statement cannot support current propagation closure.

**Historical result: `docs/education/` was Rule 7 clean and its claims were checked against the
2026-07-29 correction (`c7eb7f4`, `8b34535`, `5625ad4`).** The site predates Rule 7 and had never been
...
`log10`, and these charts used `Math.log`, printing 5 `sigma_0` equalities
(3.70/6.25/8.46/9.93/18.62 °C) instead of the three base-10 equalities
(approximately 3.08/8.07/24.73 °C). Those are input-function—and, for unit M1 prefactors under one shared positive
field, coefficient-order—locations, not habit transitions. The historical edit was made in
...
> **HISTORICAL / SUPERSEDED PHASE 6 RECORD (2026-07-26 through 2026-07-27).** The block below
> preserves the then-current launch and freeze chronology; it is not the current project state.
> Later audits invalidated the gate reading and the active science-first completion plan replaces
> these scheduling statements. Inline corrections narrow scientific claims that would otherwise
> remain false even as history.
...
**Historical launch record:** the 204-point validation sweep was launched 2026-07-27 22:47 at
`1366403`, protocol `9aa2e7c1`, concurrency 6, with an estimated ~14 h runtime. Its later artifact
is measured-only and not current gate evidence.

At that launch, every then-known prerequisite was recorded as closed. Later adversarial audits found
unmet charter and protocol obligations, so that closure did not survive review. Sections B and C landed at `9a727db`: the stale comment claiming
...
> conclusion is unsupported and withdrawn; only the sampled flip census survives at measured
> scope.
...
amendments, with `PHASE6_PROTOCOL_REVISIONS` keeping the history. At that historical point the
amendments were free because no sweep had yet run. Phase 5 was closed and pushed (`d8a8a39`); the
Phase 6 plan is registered at [phase-6-nakaya-validation.md](plans/phase-6-nakaya-validation.md)
and the historical plan prohibited a validation sweep until WP0's freeze landed. WP1 is done (`research/nakaya-morphology-diagram.md`: boundaries −3.3/−9.9/−21.5 °C ±0.5, a non-uniform temperature axis, and a water-saturation cross-check that passes on position and fails on scale).
...
WP0 calibration had already caught three things by running rather than reading: the registered surface policy was ADR 0009's `v4` while every run uses `v5`; the σ ladder cannot be computed from `sigmaWater()`, whose difference form goes negative warmer than about −3 °C; and **the habit metric is degenerate at a small measurement size** — five physically different runs all returned `AR = 0.740` (a v5-era measurement whose direction is expected to hold but whose values are being re-measured). That last one, plus CPU run costs of 191–289 s at a *degenerate* size, means the measurement size and grid cannot be frozen from CPU probes alone. **The plan was rewritten on 2026-07-26 against the operator's stated priority — accuracy to reality and science first, speed secondary.** At the executed 28³ probe the GPU path ran 6× slower than the oracle and did not meet the frozen `divTol = 1e-7`; that measured configuration did not support the proposed production role. The historical sweep therefore ran on the float64 CPU oracle, parallel across cores, while the GPU stayed diagnostic. Numerical verification moved before the grid freeze because 28³ gave a needle where 96³ gave a plate at identical conditions; the 1D spherical reference solver returned as an analytic discretization check for an idealized boundary-value problem, not an absolute physical-accuracy certificate; full parameterizations and the ±25% digitized-anchor bands were to be evaluated rather than treating `sigma_0` crossings as habit uncertainty; and a cross-platform reproducibility control was registered because Math.exp/log/pow are not correctly rounded and a habit class that flips between executed architectures is a fragility finding. The pre-freeze source corrections
...
**WP3b is closed (`7d821ee`) and it identified a source-equation erratum.** The 1D spherical reference
(`solver-cpu/src/spherical-reference.ts`) is an analytic numerical check for the same idealized
spherical boundary-value problem; it does not establish absolute physical accuracy for faceted 3-D
crystals. Transcribing monograph
Eqs. 3.16–3.24 and 3.33–3.36 from the page renders exposed an **erratum in the printed Eq. 3.35**:
its denominator should be the attachment coefficient, not `X_0/R`. Three mutually consistent
cross-checks of the same equation reject the printed form — Eq. 3.33 must reduce to Eq. 3.17 as the shell
recedes (the printed form yields the exact complement); a separately derived exact solve of the same
...
**WP3 is closed only as historical reconnaissance and is superseded for R15;** see
[phase6-convergence.md](../research/phase6-convergence.md). Four studies ran under v6 + monopole
matching, but their sparse rows and composition-changing spacing ladder do not register R15's domain,
size, timestep, or continuum controls. R15 later failed both N = 48 and N = 64 at three of four
registered domain checks. The historical observations remain useful only at their executed scope:
...
- **Domain, and a sequencing error that cost correctness rather than confidence.** The first ladder
  ...
  had equal final two samples (5185 → 5161 → 5161 → 5159 → 5159), and warm was bit-identical at
  all five domains. That two-row result does not establish an exact asymptote or transfer to R15.
- **The historical spacing ladders changed composition and do not establish grid convergence.**
  The first off-size ladder showed an apparent coarse-grid cold class difference, but the
  registered-size ladder did not reproduce that flip. From 0.35 to 0.2333 µm its composed rows moved
  `AR` +10.6% cold and +18% warm. Because spacing also changed the represented seed, achieved size,
  and stopping composition, neither movement isolates a grid effect or defines a continuum class.
  R15 must hold physical composition fixed.
...
**Historical WP0c state on 2026-07-27:** nineteen of the twenty-one freeze rows then carried a
...
- **Uncertainty was reported as class robustness, not as an interval on `AR`.** The historical
  composition-changing ladder did not establish grid convergence, so a bar around `AR` would imply
  a precision the study itself denies. Every point
...
- **Historical state at the time (2026-08-01): the then-current
  `docs/libbrecht-parameters.md` revision was frozen and enforced by content hash**, recomputed from
  the live file by the test suite and LF-normalized for cross-platform verification. The corrected
  table was then an explicitly unfrozen ADR 0040 candidate with a deliberate pin failure. Current
  authority is accepted ADR 0040/v1.19 and the separately named current-table content pin above;
  this archival paragraph does not restore the pre-acceptance state or freeze R15.

**The Tier 1 cross-platform control has two tracked self-identified fixture tables from
2026-07-31 and they split; Tier 2 survives only as a historical report**
([docs/phase6-cross-platform-control.md](phase6-cross-platform-control.md) §Result). Tier 1 is a
millisecond libm fingerprint — 448 bitwise float64 entries covering every named quantity at integer
T = −2…−30 °C plus three boundaries/fixture points. It omits the registered −31…−35 °C cold tail
and tail anchor (R29); R15 must fingerprint its entire frozen grid. Tier 2 is the end-to-end habit
class at the sweep's own configuration; its
...
> Both fixture headers self-report the same Node/V8 build (v24.13.1, V8
> 13.6.233.17-node.40) and distinct `win32/x64` versus `darwin/arm64` identities. Those headers are
> tracked provenance, not independent hardware authentication.
>
> **The historical Tier 2 table reports exact reproduction at all four ADR 0032 points** — same
> steps, attached count, aspect ratio and habit class. Its underlying arm64 logs and exit records
> were gitignored, never promoted, and are unavailable in this repository. The normalized table is
> preserved, but the end-to-end result is not independently rederivable from current bytes.
>
> So: the published bytes establish a 9/448 Tier 1 difference up to 31 ULP. They do **not** presently
> establish end-to-end habit-class portability, M1 portability, the other 200 CAK points, or a third
> architecture. R15 must publish normalized Tier 2 outputs and exit records under `evidence/`.
...
**Historical 2026-07-27 scheduling state:** two rows then remained, **Δx** and the **σ axis**, and
a finer-grid cost probe (Δx = 0.2333 µm, 72³, extent 32) was running. The coarse-grid cost was measured: eight
...
on every point. It was not a free pick — WP3 §4 did not establish convergence on that axis —
...
Under that historical plan, WP0's remainder was **WP0c**, sequenced *after* WP3's convergence
...
before the freeze rather than after it. **This target statement is superseded.** The classical
Nakaya diagram remains the qualitative in-sample report card, but it supplies no physical size
stratum. TAX2's electric-needle grid is also in-sample for M1 and geometry-mismatched; it is a blind
segmentation/reconstruction candidate, not an independent quantitative target. The current
source-lock has `passEligible=false` and no apples-to-apples held-out target. Nothing in Phase 5
authorizes a physical-validation or calibration claim; the GPU
...
**Historical maker-directed stretch register (2026-07-27):**
[stretch-sharing-and-investigation.md](stretch-sharing-and-investigation.md) recorded four
candidate external-sharing items and a post-sweep investigation agenda. It was non-binding,
granted no evidence claims, and left all external contact maker-gated. The later Rule 12 source
audit independently verified the three Libbrecht papers and found same-lineage closed forms and
source-fit values; ADR 0040 now proposes a corrected table and full re-freeze/re-run. The original
“cheap before the first sweep” scheduling window has passed.

> **END HISTORICAL / SUPERSEDED PHASE 6 RECORD.** Resume from the active state and next step above,
> not from this preserved chronology.
