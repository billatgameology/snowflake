...
6. the interface: an `AttachmentRule` is a **surface-operator interface**, wider than a per-cell
   `shouldAttach` — it owns per-cell state and mediates the mass exchange.
...
hash); and the uncertainty-reporting scheme.
...
- Doc debt created deliberately: gg-machinery's "only step that is physics" framing and the
  noise-necessity claim are softened (attachment is the only *physically parameterized* step;
  diffusion is physical, while κ, μ, hole-filling, and noise are phenomenological); these
  corrections land in the files as sessions touch them, tracked in PROGRESS.
