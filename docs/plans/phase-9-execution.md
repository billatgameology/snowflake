...
This plan succeeds the frozen proposed consumer draft
[`phase-9-modular-physics-arms.md`](phase-9-modular-physics-arms.md). That draft is an input to the
design history, not the execution authority. Phase 8 and the subsequent knowledge baseline found
...
- Phase 8B's final artifact index and 51-record successor target book are the measurement entry
...
- The branch is `phase/9` and the isolated worktree is
  ...
  timing-sensitive row is running.
- Phase 9 outputs use new `evidence/phase9-*` directories and new Phase 9 NAS roots. They never
  rewrite a Phase 6, Phase 7, or Phase 8 artifact. Shared repository indexes are changed only after
  rebasing the latest `main`, with the other phase's state retained verbatim.
- Later Phase 6 or Phase 7 results never silently replace a pinned Phase 9 input. Importing a later
  published artifact requires a named plan amendment and preserves the earlier result.
...
Before inspecting a deciding model output, append or link a frozen protocol that names:

1. exact source records and NAS byte identities, lineage, rights, uncertainty, and development role;
2. represented geometry, support, pressure, forcing, transport, ensemble, and observable, with an
...
Raw observations stay separate from coalescing, interpolation, differentiation, fitting, and model
output. A long series cannot dominate a family score merely because it contains more sampled rows.
...
- Held-out validation or a quantitative-validation claim from any Phase 8B or Phase 9 row.
- Treating the 51 priority records as a globally complete literature corpus.
- Choosing a module, threshold, interpolation, or score after seeing the best result.
- Treating a fitted attachment input, same-lineage replay, or software ablation as independent
  physical confirmation.
- Rewriting solver defaults, permanent controls, checkpoint meanings, Phase 6/7 work, or frozen
  Phase 8 bytes.
- A full thermal-fluid or phase-field solver unless a later maker-approved ADR adds it.
- Education-site work while its standing Phase 6 freeze remains in force.
...
- **Wait for Phase 6 merely because it has a lower number.** Rejected: S0/S1/S3 consume completed
  Phase 8 inputs. Isolation and host ownership address the real dependency and contention risks.
- **Start with a 3-D mechanism.** Rejected: scalar transfer and planar-facet observations can reject
  assumptions much more cheaply, and the Windows oracle host remains occupied by Phase 6.
- **Carry Phase 8A held-out labels forward.** Rejected by decision 0048: every successor record
  shaped Phase 9 and is development evidence.
