...
| 4 | Hollowing emerges with no explicit hollow rule, reproducibly across seeds — **run twice, once per `SurfaceOperator` implementation**: pass A (`GGThreshold`) is **blocking**, pass B (`LibbrechtKinetics`) is **diagnostic** (v1.3) — a failed pass B is a finding, not a blocker for Phase 6 | 🔶 **v2 implementation review pending** — v1 remains an honest immutable failure: `ad04e87` flagless `gate4` exit 1; unchanged replay proved dendrite `tExtent=99` while the dependent compact comparator reached its ordinary far-field stop at 83, long before cap. Decision 0012 replaces that cross-run geometry dependency with the same `2*rho/3` threshold, cadence, and first-crossing rule for both runs; numeric morphology parameters and thresholds remain frozen. Independent protocol review was CLEAN; authority freeze `9c9dd5a45eafb80f3e547298494f005a73d19086`. Implementation candidate `90f19a786d113f673815d4cf75459e3177806c94` passes 770/770 tests, both typechecks, Rule 7, and the app build, but is not accepted until independent adversarial review closes CLEAN. No v2 evidence exists yet. |
...
exact A-v2/B-v1 split. Exact root `npm test` passed 770/770 tests, both typechecks and Rule 7 are
clean, and the 33-module app build passed. The next serial action is an independent adversarial
review/fix loop on that immutable candidate; only a CLEAN result permits controls or evidence.
Do not calibrate, alter any morphology parameter or
