...
| 4 | Hollowing emerges with no explicit hollow rule, reproducibly across seeds — **run twice, once per `SurfaceOperator` implementation**: pass A (`GGThreshold`) is **blocking**, pass B (`LibbrechtKinetics`) is **diagnostic** (v1.3) — a failed pass B is a finding, not a blocker for Phase 6 | 🔶 **v2 criteria frozen; implementation pending** — v1 remains an honest immutable failure: `ad04e87` flagless `gate4` exit 1; unchanged replay proved dendrite `tExtent=99` while the dependent compact comparator reached its ordinary far-field stop at 83, long before cap. Decision 0012 replaces that cross-run geometry dependency with the same `2*rho/3` threshold, cadence, and first-crossing rule for both runs; numeric morphology parameters and thresholds remain frozen. Independent protocol review was CLEAN and the authority freeze is `9c9dd5a45eafb80f3e547298494f005a73d19086`. No v2 evidence exists yet. |
...
`9c9dd5a45eafb80f3e547298494f005a73d19086`; this metadata-only follow-up records it without
changing a criterion. The next serial action is the bounded manifest/runner/app version delta,
followed by full checks and an independent adversarial implementation review. Do not calibrate,
alter any morphology parameter or
