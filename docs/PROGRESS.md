...
- **Phase 4 is active under maker-directed decision
  ...
  checkpoint wording conflict. All now have fixes in the current loop; same-reviewer re-review
  continues. No Phase 4 feature implementation has begun.
...
| 4 | Hollowing emerges with no explicit hollow rule, reproducibly across seeds — **run twice, once per `SurfaceOperator` implementation**: pass A (`GGThreshold`) is **blocking**, pass B (`LibbrechtKinetics`) is **diagnostic** (v1.3) — a failed pass B is a finding, not a blocker for Phase 6 | 🔶 **in progress, criteria pre-registered** — isolated worktree/plan; v4 integration `b080654`; three WP0 review rounds found criteria/solver defects before feature work, including negative-supersaturation, solid-column, noisy-symmetry, and applied-noise witness blockers. Fixes are green at 278/278 and await same-reviewer re-review. No Phase 4 gate result exists. |
...
`e567767`. The next serial action is the same
