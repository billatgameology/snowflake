...
- **Phase 4 is active under maker-directed decision
  ...
  tree passes 394/394 tests and the app build; same-reviewer round 3 is next. No solver, app, or
  external evidence artifact changed in WP1.
...
| 4 | Hollowing emerges with no explicit hollow rule, reproducibly across seeds — **run twice, once per `SurfaceOperator` implementation**: pass A (`GGThreshold`) is **blocking**, pass B (`LibbrechtKinetics`) is **diagnostic** (v1.3) — a failed pass B is a finding, not a blocker for Phase 6 | 🔶 **in progress, criteria frozen** — final freeze `e567767`; WP0 CLEAN. WP1 round-2 seed/cap findings are repaired; 394/394 tests and app build pass, round-3 re-review next. No Phase 4 gate result exists. |
...
tests plus the app build. The next serial action is same-reviewer WP1 round 3. Loop until zero
