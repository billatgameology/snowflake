...
- **Phase 4 is active under maker-directed decision
  ...
  sets. It awaits an immutable commit and same-reviewer round 4; WP3 remains unaccepted. No real
  `gate4a`, `gate4b`, or `gate4` run and no canonical Phase 4 evidence artifact exists yet.
...
serial action is to commit the now-green identity-bound publication repair and return that exact
commit to the same reviewer for round 4. Its IPC-controlled tests make both timing seams fail
closed without writing test markers, and current app verification is 195/195 plus the build and
both full-resolution harnesses. Only after CLEAN may the remaining Phase 2a/gate3 controls run
before any real Phase 4 gate command. Work
