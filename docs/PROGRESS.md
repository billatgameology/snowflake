...
- **Phase 4 is active under maker-directed decision
  ...
  zero blockers/should-fixes, 278/278 tests, and a green app build. WP1 is next.
...
| 4 | Hollowing emerges with no explicit hollow rule, reproducibly across seeds — **run twice, once per `SurfaceOperator` implementation**: pass A (`GGThreshold`) is **blocking**, pass B (`LibbrechtKinetics`) is **diagnostic** (v1.3) — a failed pass B is a finding, not a blocker for Phase 6 | 🔶 **in progress, criteria frozen** — final freeze `e567767`; WP0 v4 integration plus four-round adversarial loop CLEAN, 278/278 tests, app build green, Phase 2a canonical hash preserved. WP1 is next; no Phase 4 gate result exists. |
...
green, Phase 3 unchanged. The next serial action is WP1: delegate pure metrics, schedule
evaluator, stable gate verdict/report types, and the registered adversarial fixtures; then send
that bounded diff to a different reviewer and loop until clean. Work only in
