...
## R01 — A cloud disappears into a crystal
...
Keep the praised four-part overview and narrated focus. Action precedes each name; labels remain available but the comparison is not a memorisation exercise. Return emphasis to deposition before leaving. Ask: **Which picture adds water without a liquid drop landing?**

Source: Ch1 `#not-a-raindrop`, `#two-words`. Rime remains a genuine liquid-impact route, not a contradiction erased from the story.

## R03 — What is white, and what is invisible?
...
Recognisable cloud → labelled droplets → labelled gas between them. Distinguish molecule and droplet scales without drawing an unexplained snow crystal in the gas close-up. Show condensation without a balance gauge: that concept is not taught yet. Keep the micrometre conversion in the optional scale annotation. Ask: **Is the white mist the water vapour? What occupies the gaps?**

Source: Ch1 `#into-the-cloud`; Ch4 `#where-the-water-is`. No “air can hold only so much” metaphor, mandatory pressure explanation, or unexplained reservoir term here.

## R04 — Ice has to get started
...
Circle lens with handle, parent selection and leader; never an unlabeled second sphere. Inside: irregular non-ice speck, translucent liquid, hatched local solid patch, then freezing. Keep the same frozen seed afterwards. Ask: **What is the speck, and what grows around it? Does below zero guarantee instant ice?**

Source: Ch1 `#supercooling`, `#not-a-raindrop`. Optional example text: “Libbrecht describes very clean droplets remaining liquid until nearly −40 °C, and certain bacterial proteins helping ice start as warm as about −2 °C. These depend on starting conditions and are not assigned freeze temperatures for all cloud droplets.” Retain the qualified source detail; do not make remembering two numbers necessary for following the seed.

## R05 — Still in size, busy at the surface
...
Preserve the praised solid region, fixed rear edge, before-line, two-way traffic and perceptible boundary movement. Hold equal exchange before naming equilibrium. At the prediction, pause the narration while background exchange continues; reveal growth afterwards. Removal is a separate deliberate perturbation, not simultaneous with addition. Ask: **At equilibrium, did the molecules stop? What changes if more vapour is added at the same temperature?**

Source: Ch4 `#what-saturated-means`; AMS equilibrium definition. Surface curvature/impurity refinements are outside this bulk reference thought experiment. Saturation terminology is deliberately deferred to R08, after the balance can be used.

## R06 — The same air, two different outcomes
...
## R07 — Follow the water all the way
...
Same cloud and grown crystal, not a restart. Ice gain → locally reduced vapour → fewer returns to liquid → net liquid loss → replenishment. Retain reciprocal paths and a wandering highlighted tracer reaching actual ice. Ask: **What makes the drop lose water as the ice grows?** No new technical name required.

Source: Ch1 `#the-relay`; Ch4 `#eating-the-cloud`, `#where-the-water-is`. Detailed diffusion resistance remains E03.

## R08 — What “water-vapour pressure” means
...
This is a dedicated new lesson, not an overlay on R06. Reuse the box and water/other-gas identities. Show wall impacts; highlight only water's contribution without depicting a magical water-only physical pressure sensor. Then reintroduce the ice and its equal exchange. Any pressure meter is an explanatory readout, not an instrument demonstration. Ask: **If there were more nitrogen but the same water vapour at the same temperature and volume, would water-vapour pressure be the total air pressure?** Expected: no; distinguish water's contribution from total. Also ask the simpler question: **What does a larger water-vapour pressure mean under the fixed conditions we just used?**
...
Retain donor identities, separate freezing from evaporation, keep existing ice/vapour visible. No universal depletion temperature or automatic growth-stop animation. Ask: **What has run out in this example—and what has not?**

Source: Ch1 `#too-cold-to-snow`; Ch4 `#eating-the-cloud`. The source's selected cooling-cloud account and approximate −20 °C statement are preserved in the reader, not used as a prediction for every cloud.

## R11 — We have followed the water, not explained the shape
...
Leave an actual beat for reconstructing the route before giving the answer. Then quiet the transfer and select concrete shape features. Preserve the grown object. No new laboratory controls during the ending. Ask: **What can you now explain, and what is still unexplained?**

Source: Ch1 `#no-blueprint`, `#two-words`. Laboratory history remains substantive optional depth and later E07/E08 material, not an invented project experiment.

## Reader depth retained, not discarded

These are actual proposed reader passages, not placeholders for missing explanations. Their facts are not prerequisites for following the main relay. Source links remain the performed script's chapter anchors.
...
The retained table gives ice-relative excesses of 5.0%, 15.7% and 47.4% for water-balanced reference air at −5, −15 and −40 °C. The separately calculated excess vapour masses in a cubic metre are 0.162, 0.218 and 0.056 grams, respectively. A cubic metre fixes the comparison volume; the values count only vapour above ice balance, not all vapour, liquid water, or lifetime delivery to a crystal. The −40 °C row compares equilibrium properties and does not guarantee liquid droplets at that temperature.

Those masses use the Murphy–Koop pressure expressions and ideal-gas conversion documented in the performed script's **Implementation and review boundaries**. The formula's percentage at −40 °C is not exactly the printed table's 47.4%, so those two sources remain distinguished. The corresponding equilibrium-excess curve has a broad maximum near −12 °C. It is not an observed universal maximum of snowfall or crystal growth speed. Source: Ch4 `#the-peak` and the performed script's existing calculation record. No new calculation or fitted curve was made for this draft.
...
| Existing source topic | Revision disposition |
| --- | --- |
| Ch1 `#two-words`, `#not-a-raindrop` | R02 main; extended taxonomy E07, structure E02. |
| Ch1 `#into-the-cloud`; Ch4 `#where-the-water-is` | R03 main; gas identity and condensation explained, droplet size retained in one spoken unit. Air-balance reason is not borrowed before its lesson. |
